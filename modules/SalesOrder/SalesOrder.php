 ...
 * Contributor(s): ______________________________________..
 * ****************************************************************************** */
require_once('include/utils/InventoryDatesUtils.php');
require_once('include/utils/InventoryApprenantsUtils.php');
require_once('include/utils/InventoryFinanceurUtils.php');
...
class SalesOrder extends CRMEntity {
    ...
    function save_module($module) {
        ...
        /* uni_cnfsecrm */
        saveInventoryDatesDetails($this);
        saveInventoryApprenantDetails($this);
        saveInventoryFinanceurDetails($this);
        /* uni_cnfsecrm */
    }
    ...
    function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
        $monfichier = fopen('suiviinvit.txt', 'a+');
        fputs($monfichier, "\n" . " save_related_module ");
        fclose($monfichier);
        $adb = $this->db;
        ...
        foreach ($with_crmids as $with_crmid) {
            if ($with_module == 'Calendar') {
                $adb->pquery('INSERT INTO vtiger_seactivityrel VALUES(?,?)', array($crmid, $with_crmid));

                /* uni_cnfsecrm liaison session au apprenants */
                $requete_apprenant = "select * from vtiger_inventoryapprenantsrel    
    where vtiger_inventoryapprenantsrel.id = ?";
                $result_apprenant = $adb->pquery($requete_apprenant, array($crmid));
                $num_rows_apprenant = $adb->num_rows($result_apprenant);

                for ($i = 0; $i < $num_rows_apprenant; $i++) {
                    $apprenantid = $adb->query_result($result_apprenant, $i, "apprenantid");
                    $resultat = $adb->query_result($result_apprenant, $i, "resultat");
                    $inscrit = $adb->query_result($result_apprenant, $i, "inscrit");
                    $sequence_no = $adb->query_result($result_apprenant, $i, "sequence_no");

                    $requete_existe = "select id from vtiger_sessionsapprenantsrel
where id = ? and apprenantid = ?";
                    $result_existe = $adb->pquery($requete_existe, array($with_crmid, $apprenantid));
                    $num_rows_existe = $adb->num_rows($result_existe);

                    if ($num_rows_existe == 0) {
                        $query = "insert into vtiger_sessionsapprenantsrel(id,sequence_no,apprenantid,resultat,inscrit) values(?,?,?,?,?)";
                        $qparams = array($with_crmid, $sequence_no, $apprenantid, $resultat, $inscrit);
