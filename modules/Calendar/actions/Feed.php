...
class Calendar_Feed_Action extends Vtiger_BasicAjax_Action {
    ...
    protected function pullEvents($start, $end, &$result, $userid = false, $color = null, $textColor = 'white', $isGroupId = false, $conditions = '') {
        ...
        while ($record = $db->fetchByAssoc($queryResult)) {
            ...
            /* uni_cnfsecrm_mod_65 */
            $nbreApprenat = $this->getNbreApprenant($crmid);
            $item['nbreApprenat'] = $nbreApprenat;
            $result[] = $item;
        }
    }
    ...
    /* uni_cnfsecrm_mod_65 */
    protected function getNbreApprenant($crmid) {
        $db = PearDatabase::getInstance();
        $query = "SELECT COUNT(apprenantid) as nbreapprenat 
                    ...
                    WHERE id=?";
        $result = $db->pquery($query, array($crmid));
        $nbreApprenat = $db->query_result($result, 0, 'nbreapprenat');
        return $nbreApprenat;
    }
