    ...
    public static function getCalendarReminder() {
        ...
        if ($activityReminder != '') {
            ...
            $time = date('H:i', strtotime("+$activityReminder seconds", $currentTime));
             $monfichier = fopen('datedebug2.txt', 'a+');
...
/*uni_cnfsecrm - reminder recyclage*/
   ...
   /* public static function getRecyclageReminder() {
        ...
        if ($activityReminder != '') {
            ...
            $date = date('Y-m-d', strtotime("+$activityReminder seconds", $currentTime));
            $time = date('H:i', strtotime("+$activityReminder seconds", $currentTime));
            $reminderActivitiesResult = "SELECT id,sessionid, reponse_par, commentaire, date_rappel, reponse, date_etre_rappeler 
                    FROM vtiger_rappel_recyclage 
                    INNER JOIN vtiger_crmentity ON vtiger_rappel_recyclage.sessionid = vtiger_crmentity.crmid
                    WHERE vtiger_crmentity.deleted = 0 AND ((DATE_FORMAT(date_etre_rappeler,'%Y-%m-%d') <= ?) 
                    AND (TIME_FORMAT(date_etre_rappeler,'%H:%i') <= ?))
                    AND vtiger_rappel_recyclage.status = 0 LIMIT 20"; //vtiger_crmentity.smownerid = ? $currentUserModel->getId()

            $result = $db->pquery($reminderActivitiesResult, array($date, "23:00"));
            $rows = $db->num_rows($result);
            for ($i = 0; $i < $rows; $i++) {
                $recordId = $db->query_result($result, $i, 'sessionid');
                $recordModels[] = Vtiger_Record_Model::getInstanceById($recordId, 'Calendar');
            }
        }
        return $recordModels;
    ...
    /* uni_cnfsecrm - modif 68 - DE */

    function getProchainesSessions() {
        $db = PearDatabase::getInstance();
        $dateNow = new DateTime();
        $dateNow = $dateNow->format('Y-m-d');

        $query = "SELECT vtiger_activity.activityid,subject,date_start,setype,
            (SELECT count(vtiger_sessionsapprenantsrel.id) 
                    FROM vtiger_sessionsapprenantsrel 
                    where vtiger_sessionsapprenantsrel.id = vtiger_activity.activityid) as nbreapprenant 
                    FROM `vtiger_activity`
                    inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_activity.activityid
                    where vtiger_crmentity.deleted = 0 and setype = ? and vtiger_activity.date_start >= ?";

        $params = array("Calendar", $dateNow);
        $result = $db->pquery($query, $params);
        $line_nbr = $db->num_rows($result);
        $sessionProchaine = array();
        for ($i = 0; $i < $line_nbr; $i++) {
            $sessionProchaine[$i]['activityid'] = $db->query_result($result, $i, 'activityid');
            $sessionProchaine[$i]['subject'] = $db->query_result($result, $i, 'subject');
            $date_start = $db->query_result($result, $i, 'date_start');
            $date_start = new DateTime($date_start);
            $date_start = $date_start->format('d/m/Y');
            $sessionProchaine[$i]['date_start'] = $date_start;
            $sessionProchaine[$i]['nbreapprenant'] = $db->query_result($result, $i, 'nbreapprenant');
        }
        return $sessionProchaine;
    }

    /* uni_cnfsecrm - modif 68 - FI */
}
