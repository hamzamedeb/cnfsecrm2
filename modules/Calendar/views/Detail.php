...
class Calendar_Detail_View extends Vtiger_Detail_View {
    ...
    function showModuleDetailView(Vtiger_Request $request) {
        ...
        if ($moduleName == 'Events') {
            ...
            $relatedSessionFinanceurs = $recordModel->getSessionFinanceurs();            
            $relatedSessionApprenants = $recordModel->getSessionApprenants();
            $relatedSessionDates = $recordModel->getSessionDates();
            $nomformation = $recordModel->getNomformation();
            $categorieformation = $recordModel->getCategorieformation();
            $viewer->assign('ACCESSIBLE_USERS', $accessibleUsers);
            $viewer->assign('INVITIES_SELECTED', $recordModel->getInvities());
            $viewer->assign('INVITEES_DETAILS', $recordModel->getInviteesDetails());
            $viewer->assign('RELATED_SESSION_FINANCEURS', $relatedSessionFinanceurs);
            $viewer->assign('RELATED_SESSION_APPRENANTS', $relatedSessionApprenants);           
            $viewer->assign('RELATED_SESSION_DATES', $relatedSessionDates);
            $nomformation = html_entity_decode($nomformation);
            
            $viewer->assign('categorieformation', $categorieformation);
            $viewer->assign('NOM_FORMATION', $nomformation);
            
            /* uni_cnfsecrm - v2 - modif 176 - DEBUT */
            $client_id = getSingleFieldValue("vtiger_seactivityrel", "crmid", "activityid", $recordId);
            $viewer->assign('CLIENT_ID', $client_id);
            if ($client_id == 189380) {
                $relatedSessionDirection = $recordModel->getListDirectionSession();
                $viewer->assign('RELATED_SESSION_DIRECTION', $relatedSessionDirection);
            }
