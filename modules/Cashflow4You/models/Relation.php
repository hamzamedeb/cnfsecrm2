...
class Cashflow4You_Relation_Model extends Vtiger_Relation_Model {
    ...
    function updateSavedRelation($module, $id, $cashflow4youId = 0) {
        ...
        if ($inv_module != false && $module == 'Invoice' && isset($refstring) && $refstring != "") {
            ...
            foreach ($IdList AS $listid) {
                ...
                $res = $this->db->pquery($select, [$listid]);
        ...
        if ($cashflow4youId != 0 || $updateStatus == 1) {
            if (($module == 'Invoice' || $module == 'ITS4YouPreInvoice' || $module == 'CreditNotes4You') && $paid >= $total && $to_pay == 0 && $entity_status != "" && $entity_status_value != "") {
                ...
                if ('CreditNotes4You' === $module && 'Refunded' === $entity_status_value) {
                    if (class_exists('CreditNotes4You_ProductsUpdate_Helper')) {
                        CreditNotes4You_ProductsUpdate_Helper::updateInventoryProductsByStatus($id, $entity_status_value);
                    }
                }
                ...
                $resultINV = $this->db->pquery($inv_query, [$id]);
    ...
    function SavePaymentFromRelation() {
        ...
        foreach ($idlist as $invid) {
            ...
            $result1 = $this->db->pquery($slect, [$invid]);
            ...
            if ($num > 0) {
                $sum_paymentamount = 0;
                for ($i = 0; $i < $num; $i++) {
                    $sum_paymentamount += floatval($this->db->query_result($result2, $i, 'amount'));
                    echo $sum_paymentamount."\n";
                }
            ...
            $upd_arr = [$p_open_amount, $p_paid_amount];
            $upd = "UPDATE " . $entity_table . " SET p_open_amount=?, p_paid_amount=?";
            if ($module == 'Invoice' || $module == 'ITS4YouPreInvoice') {
    ...
    function updateRelationsField($cash_id) {
        ...
        $result = $this->db->pquery($query, [$cash_id]);
        $i = 0;
