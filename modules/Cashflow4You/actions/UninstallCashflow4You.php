...
class Cashflow4You_UninstallCashflow4You_Action extends Settings_Vtiger_Basic_Action {
    ...
    function process(Vtiger_Request $request) {
        ...
        if ($module) {
            ...
            $this->db->pquery('DELETE FROM vtiger_org_share_action2tab WHERE tabid=?',array($tabid));
            $this->db->pquery('DELETE FROM vtiger_parenttabrel WHERE tabid=?',array($tabid));
            $this->db->pquery('DELETE FROM vtiger_blocks WHERE tabid=?',array($tabid));
            $this->db->pquery('DELETE FROM vtiger_field WHERE tabid=?',array($tabid));
            $this->db->pquery('DELETE FROM vtiger_entityname WHERE tabid=?',array($tabid));
            $this->db->pquery('DELETE FROM vtiger_tab WHERE name=?',array($moduleName));
            $this->db->pquery('DELETE FROM vtiger_tab_info WHERE prefname=?',array($moduleName));
            $this->db->pquery('DELETE FROM vtiger_relatedlists WHERE label=?',array($moduleName));
            $this->db->pquery('DELETE FROM vtiger_freetagged_objects WHERE module=?',array($moduleName));
            ...
            $this->db->pquery("DELETE FROM `vtiger_picklist` WHERE `name` = 'cashflow4you_status'",Array());
            $this->db->pquery("DELETE FROM `vtiger_picklist` WHERE `name` = 'cashflow4you_category'",Array());
            $this->db->pquery("DELETE FROM `vtiger_picklist` WHERE `name` = 'cashflow4you_paytype'",Array());
            $this->db->pquery("DELETE FROM `vtiger_picklist` WHERE `name` = 'cashflow4you_paymethod'",Array());
            $this->db->pquery("DELETE FROM `vtiger_picklist` WHERE `name` = 'cashflow4you_subcategory'",Array());
            $this->db->pquery("DELETE FROM `vtiger_fieldmodulerel` WHERE `module` = ? OR `relmodule` = ?",Array($moduleName, $moduleName));
            $this->db->pquery("DELETE FROM `vtiger_crmentity` WHERE `setype` = ?",Array($moduleName));
            $this->db->pquery("DELETE FROM `vtiger_crmentityrel` WHERE `relmodule` = ? OR `module` = ?",Array($moduleName, $moduleName));
            $this->db->pquery("DELETE FROM `vtiger_modentity_num` WHERE `semodule` = ?",Array($moduleName));
            $this->db->pquery("DELETE FROM `vtiger_modtracker_basic` WHERE `module` = ?",Array($moduleName));
            $this->db->pquery("DELETE FROM `vtiger_modtracker_relations` WHERE `targetmodule` = ?",Array($moduleName));
