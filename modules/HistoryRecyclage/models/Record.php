 ...
 */
class HistoryRecyclage_Record_Model extends Vtiger_Record_Model {
    ...
    public function updateReminderRecyclageStatus($status = 1) {
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE vtiger_historyrecyclage set status = ? where historyrecyclageid = ?", array($status, $this->getId()));
    }
