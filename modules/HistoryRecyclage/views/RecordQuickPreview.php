...
class HistoryRecyclage_RecordQuickPreview_View extends Vtiger_RecordQuickPreview_View {
    ...
    public function getDetailPopUp($recordId) {
        ...
        $query = "SELECT vtiger_contactdetails.contact_no,concat(firstname,' ',lastname) as name,
            contactid,email,vtiger_contactdetails.phone, vtiger_activity.subject,activityid ,date_start,due_date,vtiger_account.email1
            FROM vtiger_historyrecyclage
            INNER JOIN vtiger_activity ON vtiger_historyrecyclage.session = vtiger_activity.activityid
            INNER JOIN vtiger_contactdetails ON vtiger_historyrecyclage.contacts = vtiger_contactdetails.contactid
            INNER JOIN vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
            WHERE vtiger_historyrecyclage.historyrecyclageid = $recordId";
        ...
        $date_fin = date("d-m-Y", strtotime($db->query_result($result, 0, 'due_date')));
        $detail['list']['Date Fin'] = $date_fin;
        return $detail;
    }
    ...
    public function getDetailHistorique($recordId) {
        $db = PearDatabase::getInstance();
        $query = "SELECT id, reponse_par, commentaire, date_rappel, reponse, date_etre_rappeler 
                    FROM vtiger_rappel_recyclage 
                    WHERE vtiger_rappel_recyclage.historyrecyclageid = ? order by id DESC";
