...
class Inventory_GetTaxes_Action extends Vtiger_Action_Controller {
    ...
    function process(Vtiger_Request $request) {
        ...
        foreach ($idList as $id) {
            ...
            $qty_per_unit[$id] = intval($recordModel->get('qty_per_unit'));
            $nbr_heures[$id] = intval($recordModel->get('cf_901'));
            $listpriceinter[$id] = formatNumberFr($recordModel->get('unit_price'));
            $listpriceintra[$id] = formatNumberFr($recordModel->get('cf_889'));
        ...
        foreach ($idList as $id) {
            $resultData = array(
                ...
                'nbr_jours' => $qty_per_unit[$id],
                'nbr_heures' => $nbr_heures[$id],
                'listpriceinter' => $listpriceinter[$id],
                'listpriceintra' => $listpriceintra[$id],
            );
