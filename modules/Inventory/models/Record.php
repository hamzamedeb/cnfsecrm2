...
class Inventory_Record_Model extends Vtiger_Record_Model {
    ...
    function getProducts() {
        ...
        $financement = 0;
        $montantclient = 0;
        $financement = $relatedProducts[1]['final_details']['financement'];
        $montantclient = $relatedProducts[1]['final_details']['montantclient'];

        //Updating Pre tax total
        $preTaxTotal = (float) $relatedProducts[1]['final_details']['hdnSubTotal'] + (float) $relatedProducts[1]['final_details']['shipping_handling_charge'] - (float) $relatedProducts[1]['final_details']['discountTotal_final'] - (float) $financement - (float) $montantclient;        
        ...
        $relatedProducts[1]['final_details']['financement'] = number_format((float) $financement, $numOfCurrencyDecimalPlaces, '.', '');
        $relatedProducts[1]['final_details']['montantclient'] = number_format((float) $montantclient, $numOfCurrencyDecimalPlaces, '.', '');
    ...
    function getDates() {
        $relatedDates = array();
        $listDates = getInventoryAssociatedDates($this->getModuleName(), $this->getEntity());
    ...
    function getFinanceurs() {
        $listFinanceurs = getAssociatedFinanceurs($this->getModuleName(), $this->getEntity());
        $financeursCount = count($listFinanceurs);
        $relatedFinanceurs = array();

        for ($i = 1; $i <= $financeursCount; $i++) {
            $relatedFinanceurs[$i]['vendorid' . $i] = $listFinanceurs[$i]['vendorid'];
            $relatedFinanceurs[$i]['vendorname' . $i] = $listFinanceurs[$i]['vendorname'];
            $relatedFinanceurs[$i]['pourcentage' . $i] = $listFinanceurs[$i]['pourcentage'];
            $relatedFinanceurs[$i]['montant' . $i] = $listFinanceurs[$i]['montant'];
            $relatedFinanceurs[$i]['tva' . $i] = $listFinanceurs[$i]['tva'];
            $relatedFinanceurs[$i]['ttc' . $i] = $listFinanceurs[$i]['ttc'];
            $relatedFinanceurs[$i]['adresse' . $i] = $listFinanceurs[$i]['street'];
            $relatedFinanceurs[$i]['ville' . $i] = $listFinanceurs[$i]['city'];
            $relatedFinanceurs[$i]['code_postal' . $i] = $listFinanceurs[$i]['postalcode'];
            $relatedFinanceurs[$i]['telephone' . $i] = $listFinanceurs[$i]['phone'];
        }
//        $monfichier = fopen('debug_financeur_detail.txt', 'a+');
//        fputs($monfichier, "\n" . 'related financeur '.$relatedFinanceurs);
//        fclose($monfichier);
        return $relatedFinanceurs;
    }

    function getApprenants() {
        $listApprenants = getAssociatedApprenants($this->getModuleName(), $this->getEntity());
...
//        fclose($monfichier);
    	...
    	/**
	 * Funtion to get Avoir Record Url
	 * @return <String>
	 */
	public function getAvoirRecordUrl() {
		$module = $this->getModule();
		return 'index.php?module=' . $this->getModuleName() . '&view=' . $module->getEditViewName() . '&record=' . $this->getId() . '&isDuplicate=true&isAvoir=true';

	}
        
            /**
     * Function to get URL for Export the Convocation
     * @return <type>
     */
    public function getExportCONVOCATION() {
        return "index.php?module=" . $this->getModuleName() . "&action=ExportCONVOCATION&record=" . $this->getId();
    }
    
                /**
     * Function to get URL for Export the Emargement
     * @return <type>
     */
     ...
     * @return <type>
     */
    public function getExportSATISFACTION() {
        return "index.php?module=" . $this->getModuleName() . "&action=ExportSATISFACTION&record=" . $this->getId();
    }    
