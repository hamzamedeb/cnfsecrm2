...
class Inventory_Detail_View extends Vtiger_Detail_View {
    ...
    function showLineItemDetails(Vtiger_Request $request) {
        ...
        $sessionId = $this->getSessionRelConvention($record);
        $viewer->assign('SESSION_ID', $sessionId);
...
//		$viewer->view('LineItemsDetail.tpl', 'Inventory');
    ...
    /* uni_cnfsecrm - v2 - modif 94 - DEBUT */
    public function getSessionRelConvention($id)
    {
        global $adb;
        $query = 'select session from vtiger_salesorder where salesorderid = ?';
        $params = array($id);
        $result = $adb->pquery($query, $params); 
        $sessionId = $adb->query_result($result, 0, 'session');
        return $sessionId;
    }
    /* uni_cnfsecrm - v2 - modif 94 - FIN */ 
