...
Class Inventory_Edit_View extends Vtiger_Edit_View {
    ...
    public function process(Vtiger_Request $request) {
        ...
        if ($request->has('totalProductCount')) {
            ...
            /* uni_cnfsecrm - modif 92 - Interdire Générer avoir pour une facture qui a un avoir - DEBUT */
            $recordModel = Vtiger_Record_Model::getInstanceById($record);
            ...
            $parentRecordModel = Inventory_Record_Model::getInstanceById($referenceId);
            /* cnfsecrm - récupéré le contenu de la convention ou devis parent */
            $currencyInfo = $parentRecordModel->getCurrencyInfo();
            $taxes = $parentRecordModel->getProductTaxes();
            $relatedProducts = $parentRecordModel->getProducts();

            $relatedApprenants = $parentRecordModel->getApprenants();
            $relatedDates = $parentRecordModel->getDates();
            $relatedFinanceurs = $parentRecordModel->getFinanceurs();
        ...
        if (!empty($record)) {
            /* cnfsecrm */
            $relatedApprenants = $recordModel->getApprenants();
            $relatedDates = $recordModel->getDates();
            $relatedFinanceurs = $recordModel->getFinanceurs();
        }
        ...
        $viewer->assign('RELATED_DATES', $relatedDates);
        $viewer->assign('RELATED_APPRENANTS', $relatedApprenants);
        $viewer->assign('RELATED_FINANCEURS', $relatedFinanceurs);
