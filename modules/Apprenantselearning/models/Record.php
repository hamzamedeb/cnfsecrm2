 ...
 */
class Apprenantselearning_Record_Model extends Vtiger_Record_Model {
     ...
     */
    public static function getSearchResult($searchKey, $module = false, $accountid) {
        ...
        WHERE label LIKE ? AND vtiger_crmentity.deleted = 0 ';
            ...
            if ($accountid != "") {
                $query .= "AND vtiger_contactdetails.accountid = ?";
                $params = array("%$searchKey%", $accountid);
            } else {
    ...
    public function getDetailApprenant() {
        /* uni_cnfsecrm_elearning - 003 */
        $adb = PearDatabase::getInstance();
        $detailApprenant = array();
        $id = $this->getId();
        $queryDetailApprenant = $adb->pquery("select vtiger_activity.subject,statut,activityid, vtiger_service.servicecategory,serviceid,
            vtiger_contactdetails.contactid,firstname,lastname, email, phone,
            vtiger_apprenantselearningcf.cf_1218,cf_1242,vtiger_apprenantselearning.apprenant ,cf_1246 
            FROM vtiger_apprenantselearning 
            INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_apprenantselearning.apprenant
            INNER JOIN vtiger_activity on vtiger_activity.activityid = vtiger_apprenantselearning.session
            INNER JOIN vtiger_apprenantselearningcf on vtiger_apprenantselearningcf.apprenantselearningid = vtiger_apprenantselearning.apprenantselearningid
            LEFT JOIN vtiger_service on vtiger_service.serviceid = vtiger_activity.formation
            WHERE vtiger_apprenantselearning.apprenantselearningid = " . $this->getId());
        $detailApprenant['detail']['Prenom'] = $adb->query_result($queryDetailApprenant, 0, 'firstname');
        $detailApprenant['detail']['Nom'] = $adb->query_result($queryDetailApprenant, 0, 'lastname');
        $detailApprenant['detail']['email'] = $adb->query_result($queryDetailApprenant, 0, 'email');
        $detailApprenant['detail']['Telephone'] = $adb->query_result($queryDetailApprenant, 0, 'phone');
        $detailApprenant['detail']['Session'] = $adb->query_result($queryDetailApprenant, 0, 'subject');
        $detailApprenant['detail']['Statut'] = html_entity_decode($adb->query_result($queryDetailApprenant, 0, 'statut'));
        $dateRendezVous = $adb->query_result($queryDetailApprenant, 0, 'cf_1218');
        $detailApprenant['servicecategory'] = $adb->query_result($queryDetailApprenant, 0, 'servicecategory');
        if ($dateRendezVous == '') {
            $detailApprenant['detail']['Date Rendez-Vous'] = '';
        } else {
            $dateRendezVous = DateTime::createFromFormat('Y-m-d', $dateRendezVous);
            $dateRendezVous = date('d-m-Y', strtotime($dateRendezVous->format("Y-m-d H:i:s")));
            $detailApprenant['detail']['Date Rendez-Vous'] = $dateRendezVous;
        }
        $heureRendezVous = $adb->query_result($queryDetailApprenant, 0, 'cf_1246');
        $detailApprenant['detail']['Heure Rendez-Vous'] = $heureRendezVous;
        $rappel = $this->getRappel();
        $detailApprenant['rappel'] = $rappel;

        if ($rappel['type'] == 7) {
            $detailApprenant['detail']['Rappel Email'] = $rappel['email7'];
            $detailApprenant['rappelTel'] = $rappel['tel7'];
            $description = "Rappel de formation - 7 jours de formation";
        } else if ($rappel['type'] == 14) {
            $detailApprenant['detail']['Rappel Email'] = $rappel['email14'];
            $detailApprenant['rappelTel'] = $rappel['tel14'];
            $description = "Rappel de formation - 14 jours de formation";
        } else if ($rappel['type'] == 21) {
            $detailApprenant['detail']['Rappel Email'] = $rappel['email21'];
            $detailApprenant['rappelTel'] = $rappel['tel21'];
            $description = "Rappel de formation - 21 jours de formation";
        }
        if ($detailApprenant['detail']['Statut'] != 'Stagiaire à inscrire') {
            $detailApprenant['subject'] = 'Rappel de Formation';
            $detailApprenant['description'] = $this->getTemplateRappel($detailApprenant['servicecategory']);
        } else if ($detailApprenant['detail']['Statut'] == 'Stagiaire à inscrire') {
            $detailApprenant['subject'] = 'Accès formation en ligne';
            $detailApprenant['description'] = $this->getTemplateInscription($detailApprenant['servicecategory']);
        }
        $detailApprenant['theoriqueCase'] = $adb->query_result($queryDetailApprenant, 0, 'cf_1242');

        $detailApprenant['apprenant'] = $adb->query_result($queryDetailApprenant, 0, 'apprenant');
        $detailApprenant['activityid'] = $adb->query_result($queryDetailApprenant, 0, 'activityid');
        return $detailApprenant;
    }

    function getRappel() {
        $adb = PearDatabase::getInstance();
        $query = $adb->pquery("select cf_1220,cf_1224,cf_1226,cf_1236, cf_1230,cf_1232 ,cf_1238
            from vtiger_apprenantselearningcf 
                where apprenantselearningid = " . $this->getId());
        $debutFormation = $adb->query_result($query, 0, 'cf_1220');

        $rappelEmail7 = $adb->query_result($query, 0, 'cf_1224');
        $rappelEmail14 = $adb->query_result($query, 0, 'cf_1226');
        $rappelEmail21 = $adb->query_result($query, 0, 'cf_1236');

        $rappelTel7 = $adb->query_result($query, 0, 'cf_1230');
        $rappelTel14 = $adb->query_result($query, 0, 'cf_1232');
        $rappelTel21 = $adb->query_result($query, 0, 'cf_1238');

        $debutFormation = DateTime::createFromFormat('Y-m-d', $debutFormation);
        $dateNow = new DateTime();
        $diffDate = date_diff($debutFormation, $dateNow)->days;
        if ($diffDate > 7 && $diffDate <= 14) {
            $rappel['type'] = 7;
        } else if ($diffDate > 14 && $diffDate <= 21) {
            $rappel['type'] = 14;
        } else if ($diffDate > 21) {
            $rappel['type'] = 21;
        } else {
        ...
        $rappel['email7'] = $rappelEmail7;
        $rappel['email14'] = $rappelEmail14;
        $rappel['email21'] = $rappelEmail21;

        $rappel['tel7'] = $rappelTel7;
        $rappel['tel14'] = $rappelTel14;
        $rappel['tel21'] = $rappelTel21;

        return $rappel;
    }

    function getTemplateInscription($typeFormation) {
        $adb = PearDatabase::getInstance();
        ...
        $query = $adb->pquery("select body from vtiger_emailtemplates where templateid = ?", array($templateid));
        $body = $adb->query_result($query, 0, 'body');
        return $body;
    }

    function getTemplateRappel($typeFormation) {
        $adb = PearDatabase::getInstance();
        $templateid = 39;
        $query = $adb->pquery("select body from vtiger_emailtemplates where templateid = ?", array($templateid));
        $body = $adb->query_result($query, 0, 'body');
