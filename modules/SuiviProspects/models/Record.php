 ...
 */
class SuiviProspects_Record_Model extends Vtiger_Record_Model {
    ...
    public function getDetailApprenant() {
        ...
        $queryDetailApprenant = $adb->pquery("select vtiger_activity.subject,statut,activityid, vtiger_service.servicecategory,serviceid,
            ...
            vtiger_apprenantselearningcf.cf_1232,cf_1270,vtiger_apprenantselearning.apprenant  
        ...
        $dateRendezVous = $adb->query_result($queryDetailApprenant, 0, 'cf_1232');
        $detailApprenant['servicecategory'] = $adb->query_result($queryDetailApprenant, 0, 'servicecategory');
        if ($dateRendezVous == '') {
            $detailApprenant['detail']['DateRendezVous'] = '';
        } else {
            ...
            $detailApprenant['detail']['DateRndezVous'] = $dateRendezVous;
        }
        ...
        if ($detailApprenant['detail']['statut'] != 'Stagiaire à inscrire') {
            ...
            $detailApprenant['description'] = "Rappel de formation";
        ...
        $detailApprenant['theoriqueCase'] = $adb->query_result($queryDetailApprenant, 0, 'cf_1270');
    ...
    function getRappel() {
        $adb = PearDatabase::getInstance();
        $query = $adb->pquery("select cf_1234,cf_1256,cf_1258,cf_1260, cf_1262,cf_1264 ,cf_1266
            from vtiger_apprenantselearningcf 
                where apprenantselearningid = " . $this->getId());
        $debutFormation = $adb->query_result($query, 0, 'cf_1234');

        $rappelEmail7 = $adb->query_result($query, 0, 'cf_1256');
        $rappelEmail14 = $adb->query_result($query, 0, 'cf_1258');
        $rappelEmail21 = $adb->query_result($query, 0, 'cf_1260');

        $rappelTel7 = $adb->query_result($query, 0, 'cf_1262');
        $rappelTel14 = $adb->query_result($query, 0, 'cf_1264');
        $rappelTel21 = $adb->query_result($query, 0, 'cf_1266');
    ...
    public function updateReminderSuiviProspectsStatus($status = 1) {
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE vtiger_suiviprospects set status = ? where 	suiviprospectsid = ?", array($status, $this->getId()));
    }
