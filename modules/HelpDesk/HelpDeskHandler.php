    ...
    function handleEvent($eventName, $entityData) {
        ...
        if ($eventName == 'vtiger.entity.aftersave.final') {
            ...
            if ($moduleName == 'HelpDesk') {
                ...
                /* uni_cnfsecrm - v2 - modif 148 - DEBUT */
                $db = PearDatabase::getInstance();
                $query = 'select vtiger_troubletickets.contact_id, parent_id ,  vtiger_contactdetails.accountid
                    FROM vtiger_troubletickets 
                    INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_troubletickets.contact_id
                    WHERE vtiger_troubletickets.ticketid =  ?';
                $params = array($ticketId);
                $result = $db->pquery($query, $params);
                $relatif = $db->query_result($result, 0, 'parent_id');
                $client = $db->query_result($result, 0, 'accountid');
                
                if ((!is_null($client)) && ($relatif == 0)){
                    $sql = "UPDATE vtiger_troubletickets SET parent_id=? WHERE ticketid=?";
                    $db->pquery($sql, array($client, $ticketId));
                } 
//                echo $relatif ." ".$client;die();
                /* uni_cnfsecrm - v2 - modif 148 - FIN */
            }
        }
    }
