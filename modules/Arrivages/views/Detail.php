...
class Arrivages_Detail_View extends Vtiger_Index_View {
    ...
    const type = ['MISEAJOUR' => "Mise à jour", 'AJOUT' => "Ajout"];

    public function getDetailHistorique($recordId) {
        $db = PearDatabase::getInstance();
        $query = "SELECT arrivageid,vtiger_products.productid,productname,type,oldqty,newqty
                    FROM vtiger_history_arrivages 
                    INNER JOIN vtiger_products on vtiger_products.productid = vtiger_history_arrivages.productid
                    WHERE vtiger_history_arrivages.arrivageid = ?";
        $params = array($recordId);
        $result = $db->pquery($query, $params);
        $line_nbr = $db->num_rows($result);
        for ($i = 0; $i < $line_nbr; $i++) {
            $detailHistorique[$i]['productid'] = $db->query_result($result, $i, 'productid');
            $detailHistorique[$i]['productname'] = $db->query_result($result, $i, 'productname');
            $detailHistorique[$i]['type'] = self::type[$db->query_result($result, $i, 'type')];
            $detailHistorique[$i]['oldqty'] = $db->query_result($result, $i, 'oldqty');
            $detailHistorique[$i]['newqty'] = $db->query_result($result, $i, 'newqty');
        }
