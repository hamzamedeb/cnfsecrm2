...
//                " or (vtiger_activity.activitytype Not in ('Emails','Task') and  vtiger_activity.eventstatus not in ('','Held')))";
        ...
        $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,                
                vtiger_contactdetails.lastname, vtiger_contactdetails.firstname,  vtiger_activity.activityid ,
                vtiger_activity.subject, vtiger_activity.activitytype, vtiger_activity.date_start, vtiger_activity.due_date,
                vtiger_activity.time_start,vtiger_activity.time_end, vtiger_crmentity.crmid,vtiger_seactivityrel.crmid as parent_id,
                vtiger_crmentity.smownerid, vtiger_crmentity.modifiedtime, vtiger_recurringevents.recurringtype,
                case when (vtiger_activity.activitytype = 'Task') then vtiger_activity.status else vtiger_activity.eventstatus end as status               
                from vtiger_contactdetails 
                INNER JOIN vtiger_sessionsapprenantsrel ON vtiger_sessionsapprenantsrel.apprenantid = vtiger_contactdetails.contactid
                INNER JOIN vtiger_activity ON vtiger_activity.activityid = vtiger_sessionsapprenantsrel.id
     ...
     */
    //unicnfsecrm_mod_30
    function get_sessions($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        $userNameSql = getSqlForNameInDisplayFormat(array('first_name' =>
            'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
        $query = "select vtiger_activity.*,vtiger_contactdetails.*,vtiger_cntactivityrel.*,vtiger_crmentity.crmid,vtiger_crmentity.smownerid, 
                    vtiger_crmentity.modifiedtime,vtiger_recurringevents.recurringtype as recurrence,vtiger_activity.eventstatus as status
                    ,vtiger_seactivityrel.crmid as parent_id
                    ...
                    left join vtiger_sessionsapprenantsrel ON vtiger_sessionsapprenantsrel.id = vtiger_activity.activityid 
                    left join vtiger_contactdetails ON vtiger_sessionsapprenantsrel.apprenantid = vtiger_contactdetails.contactid  
                    WHERE vtiger_sessionsapprenantsrel.apprenantid = " . $id . " and vtiger_crmentity.deleted=0";
...
//                 where vtiger_crmentity.deleted=0  and  vtiger_salesorder.contactid = " . $id;
        $query = "select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name, vtiger_salesorder.* , vtiger_crmentity.*,  vtiger_quotes.subject as quotename, vtiger_account.accountname
                    FROM vtiger_salesorder
                    inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_salesorder.salesorderid 
                    left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid 
                    LEFT JOIN vtiger_salesordercf ON vtiger_salesordercf.salesorderid = vtiger_salesorder.salesorderid 
                    ...
                    left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid 
                    LEFT JOIN vtiger_invoice_recurring_info ON vtiger_invoice_recurring_info.start_period = vtiger_salesorder.salesorderid
                    INNER JOIN vtiger_inventoryapprenantsrel ON vtiger_inventoryapprenantsrel.id = vtiger_salesorder.salesorderid
                    INNER JOIN vtiger_contactdetails ON vtiger_inventoryapprenantsrel.apprenantid = vtiger_contactdetails.contactid
                    WHERE vtiger_inventoryapprenantsrel.apprenantid = " . $id . " and vtiger_crmentity.deleted=0";
...
//End
    ...
    function get_services($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        $userNameSql = getSqlForNameInDisplayFormat(array('first_name' =>
            'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
        $query = "select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.crmid, vtiger_service.serviceid,vtiger_service.servicename,vtiger_service.service_no,vtiger_service.qty_per_unit,vtiger_service.unit_price,vtiger_service.commissionrate,
				vtiger_crmentity.smownerid
				FROM vtiger_service 
                                inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_service.serviceid
				INNER JOIN vtiger_activity ON vtiger_activity.formation = vtiger_service.serviceid
                                INNER JOIN vtiger_sessionsapprenantsrel ON vtiger_sessionsapprenantsrel.id = vtiger_activity.activityid
				left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
				left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
				where vtiger_crmentity.deleted=0 and vtiger_sessionsapprenantsrel.apprenantid=" . $id;
