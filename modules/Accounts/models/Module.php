...
class Accounts_Module_Model extends Vtiger_Module_Model {
    ...
    public function getRelationQuery($recordId, $functionName, $relatedModule, $relationId) {
        ...
        /* uni_cnfsecrm - modif 70 FIN */
                /* uni_cnfsecrm - v2 - modif 178 - DEBUT */
        if ($functionName == "get_contact_speciaux") {
            $query = $this->get_contact_speciaux($recordId, $functionName, $relatedModule, $relationId);
        } else if ($functionName == "get_contacts") {
            $query = $this->get_contacts($recordId, $functionName, $relatedModule, $relationId);
        } else {
            $query = parent::getRelationQuery($recordId, $functionName, $relatedModule, $relationId);
        }
        /* uni_cnfsecrm - v2 - modif 178 - FIN */
        /* } */
    ...
    function get_contact_speciaux($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        $query = "SELECT DISTINCT vtiger_crmentity.crmid,vtiger_contactdetails.firstname, vtiger_contactdetails.lastname, 
        vtiger_contactdetails.phone, vtiger_contactdetails.email, vtiger_contactdetails.accountid, vtiger_crmentity.smownerid, 
        vtiger_contactscf.cf_984 FROM vtiger_contactdetails 
        ...
        WHERE vtiger_crmentity.deleted = 0 AND vtiger_contactdetails.accountid = $id and cf_1316 = 1 ";
        return $query;
    }
    ...
    function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
        LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id 
        WHERE vtiger_crmentity.deleted = 0 AND vtiger_contactdetails.accountid = $id and cf_1316 = 0 ";
        return $query;
    }
