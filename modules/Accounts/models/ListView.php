    ...
    public function getListViewEntries($pagingModel) {
        ...
        if (!empty($orderBy) && $orderByFieldModel) {
            if ($orderBy == 'roleid' && $moduleName == 'Users') {
                ...
                /* uni_cnfsecrm - tri par account_no en supprimant CLI */
                $a = $queryGenerator->getOrderByColumn($orderBy);
                if ($a == 'vtiger_account.account_no') {
                    $caractere = 'CLI';
                    $espace = '';
                    $listQuery .= ' ORDER BY cast(REPLACE(' . $a . ', "' . $caractere . '", "' . $espace . '") as unsigned) ' . $sortOrder;
                    //ORDER BY cast(REPLACE(vtiger_account.account_no, 'CLI', '') as unsigned)
                } else {
                    $listQuery .= ' ORDER BY ' . $a . ' ' . $sortOrder;
                }
            }
