...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model {
    ...
    public function getRelatedEntriesCount() {
        global $log;
        $log->debug("Entering getRelatedEntriesCount");
        $db = PearDatabase::getInstance();
        $currentUser = Users_Record_Model::getCurrentUserModel();
        ...
        if ($position) {
            ...
            if ($relatedModuleName == 'Calendar') {
                /* uni_cnfsecrm - modif 70 DEBUT */
                if (($parentModule->get('name') != 'Accounts')) {
        ...
        /* uni_cnfsecrm - modif 70 DEBUT*/
        if (($parentModule->get('name') == 'Accounts') && ($relatedModuleName == 'Calendar')) {
            $relationQuery .= 'GROUP BY vtiger_activity.activityid';
        }
        /* uni_cnfsecrm - modif 70 FIN */ 
        
        $log->debug("relationQuery : " . $relationQuery);
