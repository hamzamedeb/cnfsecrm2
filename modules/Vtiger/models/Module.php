...
class Vtiger_Module_Model extends Vtiger_Module {
     ...
     */
    public function searchRecord($searchValue, $parentId = false, $parentModule = false, $relatedModule = false, $accountid = "", $activityid = "") {
        ...
        if (!empty($searchValue) && empty($parentId) && empty($parentModule)) {
            ...
            if ($activityid == "") {
                $matchingRecords = Vtiger_Record_Model::getSearchResult($searchValue, $this->getName(), $accountid);
            } else {
                $matchingRecords = Vtiger_Record_Model::getSearchResultApprenantsByEvents($searchValue, $this->getName(), $activityid);
            }
     ...
     */
    function getAppName() {
                $monfichier = fopen('moduledebug.txt', 'a+');
        fputs($monfichier, "\n" . ' Test03 :  ');
        fclose($monfichier);
        $db = PearDatabase::getInstance();
...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
    ...
    /* uni_cnfsecrm_elearning - 003 - DEBUT */

    public function getNbreApprAInscrir() {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery("SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0 AND statut = 'Stagiaire à inscrire'");
        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    public function getNbreApprEncourForm() {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery("SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0  AND statut = 'En cours de formation'");
        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    public function getNbreRVAPrendre() {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery("SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0  AND statut = 'Rendez-vous à prendre'");
        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    public function getNbreRVPratique() {
        $adb = PearDatabase::getInstance();
        $query = "SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0  AND statut = ? AND cf_1218 != ''";
        $result = $adb->pquery($query, array('Rendez-vous pratique'));

        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    public function getNbreRappels() {
        $adb = PearDatabase::getInstance();
        //            ((cf_1222 = ? AND cf_1224 != 'on') || (cf_1222 = ? AND cf_1226 != 'on') || (cf_1222 = ? AND cf_1236 != 'on')) AND cf_1222 != ?";
        $query = "SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0  AND statut = ? AND cf_1222 != ?";
        $result = $adb->pquery($query, array('En cours de formation', 'Pas de rappel'));
        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    public function getNbreApprFiniForm() {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery("SELECT count(vtiger_apprenantselearning.apprenantselearningid) as nbrformelearning
            FROM vtiger_apprenantselearning  
            INNER JOIN vtiger_crmentity ON vtiger_apprenantselearning.apprenantselearningid = vtiger_crmentity.crmid 
            INNER JOIN vtiger_apprenantselearningcf ON vtiger_apprenantselearning.apprenantselearningid = vtiger_apprenantselearningcf.apprenantselearningid 
            WHERE vtiger_crmentity.deleted=0  AND statut = 'Fini la formation'");
        $nbrformelearning = $adb->query_result($result, 0, 'nbrformelearning');
        return $nbrformelearning;
    }

    /* uni_cnfsecrm_elearning - 003 - FIN */
}
