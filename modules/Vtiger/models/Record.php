...
class Vtiger_Record_Model extends Vtiger_Base_Model {
     ...
     * @return <Array> - List of Vtiger_Record_Model or Module Specific Record Model instances
     */
    public static function getSearchResultApprenantsByEvents($searchKey, $module = false, $activityid) {
        $db = PearDatabase::getInstance();
        /* uni_cnfsecrm - recherche apprenants client sélectionné */
        $query = 'SELECT label, crmid, setype, createdtime FROM vtiger_crmentity
        INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_crmentity.crmid
        INNER JOIN vtiger_sessionsapprenantsrel ON vtiger_sessionsapprenantsrel.apprenantid = vtiger_contactdetails.contactid
        WHERE label LIKE ? AND id=? AND vtiger_crmentity.deleted = 0 ';

        $params = array("%$searchKey%", $activityid);

        $result = $db->pquery($query, $params);
        $noOfRows = $db->num_rows($result);
        $monfichier = fopen('debug_sessions.txt', 'a+');
        fputs($monfichier, "\n" . ' noOfRows ' . $noOfRows);
        fclose($monfichier);
        $moduleModels = $matchingRecords = $leadIdsList = array();
        for ($i = 0; $i < $noOfRows; ++$i) {
