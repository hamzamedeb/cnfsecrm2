...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
    ...
    for ($i = 1; $i <= $tot_no_prod; $i++) {
        ...
        $tarif = vtlib_purify($_REQUEST['tarif' . $i]);
        $nbrjours = vtlib_purify($_REQUEST['nbr_jours' . $i]);
        $nbrheures = vtlib_purify($_REQUEST['nbr_heures' . $i]);
        $baseheures = vtlib_purify($_REQUEST['base_heure' . $i]);
        $naturecalcul = vtlib_purify($_REQUEST['nature_calcul' . $i]);
        $parpersonne = vtlib_purify($_REQUEST['par_personne' . $i]);
        $listpriceinter = vtlib_purify($_REQUEST['listpriceinter' . $i]);
        $listpriceintra = vtlib_purify($_REQUEST['listpriceintra' . $i]);
        ...
        $query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, purchase_cost, margin,nbrjours,nbrheures,baseheures,naturecalcul,parpersonne,listpriceinter,listpriceintra,tarif)
					VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        $qparams = array($focus->id, $prod_id, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin, $nbrjours, $nbrheures, $baseheures, $naturecalcul, $parpersonne, $listpriceinter, $listpriceintra, $tarif);
        $adb->pquery($query, $qparams);
    ...
    if ($module == 'Invoice' || $module == 'SalesOrder') {
        $financement = vtlib_purify($_REQUEST['financement']);
        $updatequery .= " financement=?,";
        array_push($updateparams, $financement);
    }
    if ($module == 'Invoice') {
        $montantclient = vtlib_purify($_REQUEST['montantclient']);
        $updatequery .= " montantclient=?,";
        array_push($updateparams, $montantclient);
    }
    ...
    $name_product = getSingleFieldValue("vtiger_service", "servicename", "serviceid", $prod_id);

    /* uni_cnfsecrm */
    if ($module == "Quotes" && $prod_id != "") {
        $updatequery_numero .= "update vtiger_quotes set subject = ? where quoteid = ?";
        $updateparams_numero = array($name_product, $focus->id);
        $adb->pquery($updatequery_numero, $updateparams_numero);
    }

    if ($module == "SalesOrder" && $prod_id != "") {
        $updatequery_numero .= "update vtiger_salesorder set subject = ? where salesorderid = ?";
        $updateparams_numero = array($name_product, $focus->id);
        $adb->pquery($updatequery_numero, $updateparams_numero);
    }
