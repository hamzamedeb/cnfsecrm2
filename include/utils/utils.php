...
function debugfile($chaine) {

    $monfichier = fopen('debugfile.txt', 'a+');
    fputs($monfichier, "\n" . $chaine);
    fclose($monfichier);
}
...
if (!function_exists('split')) {
    ...
    function split($delim, $str) {
        return php7_compat_split($delim, $str);
    }
...
function getDuplicatesPreventionMessage($moduleName, $duplicateRecordsList) {
    ...
    /* unicnfsecrm - hide message pour modifier les règles de prévention en double */
...
function formatListBeforeImport($text) {
