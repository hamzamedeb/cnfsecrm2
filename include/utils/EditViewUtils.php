...
function getSessionAssociatedApprenants($module, $focus, $seid = '', $refModuleName = false) {
    ...
    $query = 'SELECT resultat,ticket_examen,ticket_examen_test, inscrit,etat, vtiger_sessionsapprenantsrel.id,apprenantid,type_tokens,type_tokens_test,
            ...
            b1v_b2v_bc_br_b1v, b1v_b2v_bc_br_b2v, b1v_b2v_bc_br_bc, b1v_b2v_bc_br_br, b1v_b2v_bc_br_essai, 
            b1v_b2v_bc_br_verification, b1v_b2v_bc_br_mesurage, b1v_b2v_bc_br_h1v_h2v_b0, 
            b1v_b2v_bc_br_h1v_h2v_h0v, b1v_b2v_bc_br_h1v_h2v_bs, b1v_b2v_bc_br_h1v_h2v_manoeuvre, 
            b1v_b2v_bc_br_h1v_h2v_b1v, b1v_b2v_bc_br_h1v_h2v_b2v, b1v_b2v_bc_br_h1v_h2v_bc, 
            b1v_b2v_bc_br_h1v_h2v_br, b1v_b2v_bc_br_h1v_h2v_essai, b1v_b2v_bc_br_h1v_h2v_verification, 
            ...
            date_start_appr , date_fin_appr , duree_jour , duree_heure,type_tokens_reaffecter,ticket_examen_reaffecter,
            CONCAT(vtiger_contactdetails.firstname, " ", vtiger_contactdetails.lastname) AS contactName,vtiger_contactdetails.email,vtiger_contactdetails.phone,
            vtiger_contactdetails.contact_no,vtiger_contactdetails.lastname,vtiger_account.account_no,accountname,vtiger_account.accountid,vtiger_account.email1,emailenligne,
            be_essai,be_mesurage,be_verification,be_manoeuvre,he_essai,he_mesurage,he_verification,he_manoeuvre,initiale,recyclage,testprerequis,electricien,cf_1208 as encompte
        ,vtiger_activitycf.cf_998, cf_996,vtiger_activity.date_start,due_date, vtiger_contactscf.cf_1320
    ...
    for ($i = 1; $i <= $num_rows; $i++) {
        ...
        $apprenants_Detail[$i]['ticket_examen_test'] = $adb->query_result($result, $i - 1, 'ticket_examen_test');
        $apprenants_Detail[$i]['inscrit'] = $adb->query_result($result, $i - 1, 'inscrit');
        $apprenants_Detail[$i]['etat'] = $adb->query_result($result, $i - 1, 'etat');
        $apprenants_Detail[$i]['apprenantid'] = $adb->query_result($result, $i - 1, 'apprenantid');
        $apprenants_Detail[$i]['email'] = $adb->query_result($result, $i - 1, 'email');
        //unicnfsecrm_mod_26
        if (($apprenants_Detail[$i]['email'] == '') || ($apprenants_Detail[$i]['email'] == null)) {
            $apprenants_Detail[$i]['email'] = $adb->query_result($result, $i - 1, 'email1');
        }
        $apprenants_Detail[$i]['phone'] = $adb->query_result($result, $i - 1, 'phone');
        $apprenants_Detail[$i]['contact_no'] = $adb->query_result($result, $i - 1, 'contact_no');
        $apprenants_Detail[$i]['nomclient'] = $adb->query_result($result, $i - 1, 'accountname');
        $apprenants_Detail[$i]['numclient'] = $adb->query_result($result, $i - 1, 'account_no');
        $apprenants_Detail[$i]['accountid'] = $adb->query_result($result, $i - 1, 'accountid');
        //unicnfsecrm_mod_56                
        $apprenants_Detail[$i]['type_tokens'] = $adb->query_result($result, $i - 1, 'type_tokens');
        $apprenants_Detail[$i]['type_tokens_test'] = $adb->query_result($result, $i - 1, 'type_tokens_test');
        $apprenants_Detail[$i]['emailenligne'] = $adb->query_result($result, $i - 1, 'emailenligne');
        ...
        /* uni_cnfsecrm_elearning - 001 */
        $apprenants_Detail[$i]['encompte'] = $adb->query_result($result, $i - 1, 'encompte');
        /* modif */
        $apprenants_Detail[$i]['b0_h0_h0v_b0'] = $adb->query_result($result, $i - 1, 'b0_h0_h0v_b0');
        $apprenants_Detail[$i]['b0_h0_h0v_h0v'] = $adb->query_result($result, $i - 1, 'b0_h0_h0v_h0v');
        $apprenants_Detail[$i]['bs_be_he_b0'] = $adb->query_result($result, $i - 1, 'bs_be_he_b0');
        $apprenants_Detail[$i]['bs_be_he_h0v'] = $adb->query_result($result, $i - 1, 'bs_be_he_h0v');
        $apprenants_Detail[$i]['bs_be_he_bs'] = $adb->query_result($result, $i - 1, 'bs_be_he_bs');
        $apprenants_Detail[$i]['bs_be_he_manoeuvre'] = $adb->query_result($result, $i - 1, 'bs_be_he_manoeuvre');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_b0'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_b0');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h0v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h0v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_bs'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_bs');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_manoeuvre'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_manoeuvre');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_b1v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_b1v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_b2v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_b2v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_bc'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_bc');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_br'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_br');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_essai'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_essai');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_verification'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_verification');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_mesurage'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_mesurage');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_b0'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_b0');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_h0v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_h0v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_bs'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_bs');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_manoeuvre'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_manoeuvre');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_b1v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_b1v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_b2v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_b2v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_bc'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_bc');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_br'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_br');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_essai'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_essai');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_verification'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_verification');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_mesurage'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_mesurage');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_h1v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_h1v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_h2v'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_h2v');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_hc'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_hc');
        /* fin */
        /**/
        $apprenants_Detail[$i]['bs_be_he_he'] = $adb->query_result($result, $i - 1, 'bs_be_he_he');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_he'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_he');
        $apprenants_Detail[$i]['b1v_b2v_bc_br_h1v_h2v_he'] = $adb->query_result($result, $i - 1, 'b1v_b2v_bc_br_h1v_h2v_he');
        /**/
//        $apprenants_Detail[$i]['cf_998'] = $adb->query_result($result, $i - 1, 'cf_998');
//        $apprenants_Detail[$i]['cf_996'] = $adb->query_result($result, $i - 1, 'cf_996');
//        $apprenants_Detail[$i]['date_start'] = $adb->query_result($result, $i - 1, 'date_start');
//        $apprenants_Detail[$i]['due_date'] = $adb->query_result($result, $i - 1, 'due_date');
        $apprenants_Detail[$i]['direction'] = $adb->query_result($result, $i - 1, 'cf_1320');
        

        $monfichier = fopen('date_debug.txt', 'a+');
        fputs($monfichier, "\n" . ' date_start_appr ' . $adb->query_result($result, $i - 1, 'date_start_appr'));
        fclose($monfichier);

        if ($adb->query_result($result, $i - 1, 'date_start_appr') == '0000-00-00' || $adb->query_result($result, 0, 'date_start_appr') == '' || $adb->query_result($result, 0, 'date_start_appr') == '1970-01-01') {
            $date_start_session = strtotime($adb->query_result($result, $i - 1, 'date_start'));
            $apprenants_Detail[$i]['date_start_appr'] = date('d-m-Y', $date_start_session);
            $monfichier = fopen('date_debug.txt', 'a+');
            fputs($monfichier, "\n" . ' t01 ' . $apprenants_Detail[$i]['date_start_appr']);
            fclose($monfichier);
        } else {
            $date_start_appr = strtotime($adb->query_result($result, $i - 1, 'date_start_appr'));
            $apprenants_Detail[$i]['date_start_appr'] = date('d-m-Y', $date_start_appr);
            $monfichier = fopen('date_debug.txt', 'a+');
            fputs($monfichier, "\n" . ' t02 ' . $apprenants_Detail[$i]['date_start_appr']);
            fclose($monfichier);
        }
        if ($adb->query_result($result, $i - 1, 'date_fin_appr') == '0000-00-00' || $adb->query_result($result, 0, 'date_fin_appr') == '' || $adb->query_result($result, 0, 'date_fin_appr') == '1970-01-01') {
            $date_fin_session = strtotime($adb->query_result($result, $i - 1, 'due_date'));
            $apprenants_Detail[$i]['date_fin_appr'] = date('d-m-Y', $date_fin_session);
        } else {
            $date_fin_appr = strtotime($adb->query_result($result, $i - 1, 'date_fin_appr'));
            $apprenants_Detail[$i]['date_fin_appr'] = date('d-m-Y', $date_fin_appr);
        }

        if ($adb->query_result($result, $i - 1, 'duree_jour') == 0) {
            $apprenants_Detail[$i]['duree_jour'] = $adb->query_result($result, $i - 1, 'cf_998');
        } else {
            $apprenants_Detail[$i]['duree_jour'] = $adb->query_result($result, $i - 1, 'duree_jour');
        }
        if ($adb->query_result($result, $i - 1, 'duree_heure') == 0) {
            $apprenants_Detail[$i]['duree_heure'] = $adb->query_result($result, $i - 1, 'cf_996');
        } else {
            $apprenants_Detail[$i]['duree_heure'] = $adb->query_result($result, $i - 1, 'duree_heure');
        }

        /* uni_cnfsecrm - v2 - modif 142 - DEBUT */
        $apprenants_Detail[$i]['type_tokens_reaffecter'] = $adb->query_result($result, $i - 1, 'type_tokens_reaffecter');
        $apprenants_Detail[$i]['ticket_examen_reaffecter'] = $adb->query_result($result, $i - 1, 'ticket_examen_reaffecter');
        /* uni_cnfsecrm - v2 - modif 142 - FIN */

        /* uni_cnfsecrm - modif 71 DEBUT */
        $queryStatutFacture = 'SELECT invoiceid,invoicestatus,salesorderid 
                                    FROM vtiger_invoice 
                                    WHERE session = ? AND accountid = ?';
        $paramsStatutFacture = array($focus->id, $apprenants_Detail[$i]['accountid']);
        ...
        $nbreFinanceur = $adb->query_result($resultNbreFinanceur, 0, 'id');

        if ($factureid > 0) {
            if ($statutFacture == 'Paid') {
                if (count($nbreFinanceur) > 0) {
                    $apprenants_Detail[$i]['statutfacture'] = 'PAYE (Financeur)';
                } else {
                    if ($apprenants_Detail[$i]['encompte'] == 1) {
                        $apprenants_Detail[$i]['statutfacture'] = 'Payée en Compte';
                    } else {
                ...
                if (count($nbreFinanceur) > 0) {
                    $apprenants_Detail[$i]['statutfacture'] = 'NON PAYE (Financeur)';
                } else {
                    if ($apprenants_Detail[$i]['encompte'] == 1) {
                        $apprenants_Detail[$i]['statutfacture'] = 'Non payée en Compte';
                    } else {
            ...
            /* uni_cnfsecrm_elearning - 002 */
            $queryNbreFinanceur2 = 'SELECT salesorderid FROM vtiger_salesorder 
INNER JOIN vtiger_inventoryfinanceurrel on vtiger_inventoryfinanceurrel.id = vtiger_salesorder.salesorderid
WHERE session = ? AND accountid = ? AND vtiger_inventoryfinanceurrel.vendorid <> ?';
            $paramsNbreFinanceur2 = array($focus->id, $apprenants_Detail[$i]['accountid'], 0);
            $resultNbreFinanceur2 = $adb->pquery($queryNbreFinanceur2, $paramsNbreFinanceur2);
            $salesorderid = $adb->query_result($resultNbreFinanceur2, 0, 'salesorderid');

            $nbreFinanceur2 = $adb->num_rows($resultNbreFinanceur2);
            if ($nbreFinanceur2 > 0) {
                $apprenants_Detail[$i]['statutfacture'] = 'Non Facturé Financeur';
            } else {
                if ($apprenants_Detail[$i]['encompte'] == 1) {
                    $apprenants_Detail[$i]['statutfacture'] = 'Non facturée en Compte';
                } else {
                    $apprenants_Detail[$i]['statutfacture'] = 'NON FACTURE';
                }
            }
        }
        /* uni_cnfsecrm - modif 71 FIN */
    }
    return $apprenants_Detail;
}
...
function getOptionsApprenants($module, $sessionid, $apprenantid) {
    global $adb;
    $apprenants_Detail = Array();
    $query = 'SELECT resultat,ticket_examen, inscrit,etat, vtiger_sessionsapprenantsrel.id,apprenantid,b0_h0_h0v_b0, b0_h0_h0v_h0v, bs_be_he_b0, bs_be_he_h0v, bs_be_he_bs, bs_be_he_manoeuvre
            ,b1v_b2v_bc_br_b0, b1v_b2v_bc_br_h0v, b1v_b2v_bc_br_bs, b1v_b2v_bc_br_manoeuvre, 
            b1v_b2v_bc_br_b1v, b1v_b2v_bc_br_b2v, b1v_b2v_bc_br_bc, b1v_b2v_bc_br_br, b1v_b2v_bc_br_essai, 
            b1v_b2v_bc_br_verification, b1v_b2v_bc_br_mesurage, b1v_b2v_bc_br_h1v_h2v_b0, 
            b1v_b2v_bc_br_h1v_h2v_h0v, b1v_b2v_bc_br_h1v_h2v_bs, b1v_b2v_bc_br_h1v_h2v_manoeuvre, 
            ...
            b1v_b2v_bc_br_h1v_h2v_br, b1v_b2v_bc_br_h1v_h2v_essai, b1v_b2v_bc_br_h1v_h2v_verification, 
            b1v_b2v_bc_br_h1v_h2v_mesurage, b1v_b2v_bc_br_h1v_h2v_h1v, b1v_b2v_bc_br_h1v_h2v_h2v, 
            b1v_b2v_bc_br_h1v_h2v_hc,b1v_b2v_bc_br_h1v_h2v_he,b1v_b2v_bc_br_he,bs_be_he_he,
            ...
            ,vtiger_activitycf.cf_998, cf_996,vtiger_activity.date_start,due_date
        FROM vtiger_sessionsapprenantsrel
        INNER JOIN vtiger_contactdetails ON vtiger_contactdetails.contactid = vtiger_sessionsapprenantsrel.apprenantid
        LEFT JOIN vtiger_account ON vtiger_account.accountid = vtiger_contactdetails.accountid
        LEFT JOIN vtiger_activitycf on vtiger_activitycf.activityid = vtiger_sessionsapprenantsrel.id
    ...
    /* modif */
    $apprenants_Detail[0]['b0_h0_h0v_b0'] = $adb->query_result($result, 0, 'b0_h0_h0v_b0');
    $apprenants_Detail[0]['b0_h0_h0v_h0v'] = $adb->query_result($result, 0, 'b0_h0_h0v_h0v');
    $apprenants_Detail[0]['bs_be_he_b0'] = $adb->query_result($result, 0, 'bs_be_he_b0');
    $apprenants_Detail[0]['bs_be_he_h0v'] = $adb->query_result($result, 0, 'bs_be_he_h0v');
    $apprenants_Detail[0]['bs_be_he_bs'] = $adb->query_result($result, 0, 'bs_be_he_bs');
    $apprenants_Detail[0]['bs_be_he_manoeuvre'] = $adb->query_result($result, 0, 'bs_be_he_manoeuvre');
    $apprenants_Detail[0]['b1v_b2v_bc_br_b0'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_b0');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h0v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h0v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_bs'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_bs');
    $apprenants_Detail[0]['b1v_b2v_bc_br_manoeuvre'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_manoeuvre');
    $apprenants_Detail[0]['b1v_b2v_bc_br_b1v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_b1v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_b2v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_b2v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_bc'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_bc');
    $apprenants_Detail[0]['b1v_b2v_bc_br_br'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_br');
    $apprenants_Detail[0]['b1v_b2v_bc_br_essai'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_essai');
    $apprenants_Detail[0]['b1v_b2v_bc_br_verification'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_verification');
    $apprenants_Detail[0]['b1v_b2v_bc_br_mesurage'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_mesurage');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_b0'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_b0');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_h0v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_h0v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_bs'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_bs');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_manoeuvre'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_manoeuvre');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_b1v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_b1v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_b2v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_b2v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_bc'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_bc');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_br'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_br');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_essai'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_essai');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_verification'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_verification');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_mesurage'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_mesurage');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_h1v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_h1v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_h2v'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_h2v');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_hc'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_hc');
    /* fin modif */

    /* uni_cnfsecrm - v2 - modif 115 - DEBUT */
    $apprenants_Detail[0]['bs_be_he_he'] = $adb->query_result($result, 0, 'bs_be_he_he');
    $apprenants_Detail[0]['b1v_b2v_bc_br_he'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_he');
    $apprenants_Detail[0]['b1v_b2v_bc_br_h1v_h2v_he'] = $adb->query_result($result, 0, 'b1v_b2v_bc_br_h1v_h2v_he');
    /* uni_cnfsecrm - v2 - modif 115 - FIN */
    //

    $monfichier = fopen('popupdebug.txt', 'a+');
    fputs($monfichier, "\n" . ' t01 ' . $adb->query_result($result, 0, 'date_start_appr'));
    fclose($monfichier);

    if ($adb->query_result($result, 0, 'date_start_appr') == '0000-00-00' || $adb->query_result($result, 0, 'date_start_appr') == '' || $adb->query_result($result, 0, 'date_start_appr') == '1970-01-01') {
        $date_start_session = strtotime($adb->query_result($result, 0, 'date_start'));
        $monfichier = fopen('popupdebug.txt', 'a+');
        fputs($monfichier, "\n" . ' t02 ' . $date_start_session);
        fclose($monfichier);
        $apprenants_Detail[0]['date_start_appr'] = date('d-m-Y', $date_start_session);
        $monfichier = fopen('popupdebug.txt', 'a+');
        fputs($monfichier, "\n" . ' t03 ' . $apprenants_Detail[0]['date_start_appr']);
        fclose($monfichier);
    } else {
        $date_start_appr = strtotime($adb->query_result($result, 0, 'date_start_appr'));
        $apprenants_Detail[0]['date_start_appr'] = date('d-m-Y', $date_start_appr);
    }

    if ($adb->query_result($result, 0, 'date_fin_appr') == '0000-00-00' || $adb->query_result($result, 0, 'date_fin_appr') == '' || $adb->query_result($result, 0, 'date_fin_appr') == '1970-01-01') {
        $date_fin_session = strtotime($adb->query_result($result, 0, 'due_date'));
        $apprenants_Detail[0]['date_fin_appr'] = date('d-m-Y', $date_fin_session);
    } else {
        $date_fin_appr = strtotime($adb->query_result($result, 0, 'date_fin_appr'));
        $apprenants_Detail[0]['date_fin_appr'] = date('d-m-Y', $date_fin_appr);
    }

    if ($adb->query_result($result, 0, 'duree_jour') == 0) {
        $apprenants_Detail[0]['duree_jour'] = $adb->query_result($result, 0, 'cf_998');
    } else {
        $apprenants_Detail[0]['duree_jour'] = $adb->query_result($result, 0, 'duree_jour');
    }
    if ($adb->query_result($result, 0, 'duree_heure') == 0) {
        $apprenants_Detail[0]['duree_heure'] = $adb->query_result($result, 0, 'cf_996');
    } else {
        $apprenants_Detail[0]['duree_heure'] = $adb->query_result($result, 0, 'duree_heure');
    }
    //var_dump($apprenants_Detail);die();        
    return $apprenants_Detail;
}
...
function getDetailSession($record) {
    global $adb;
    $query = "SELECT vtiger_activity.date_start, due_date, vtiger_activitycf.cf_998, cf_996
    ...
    $date_start = strtotime($adb->query_result($result, 0, 'date_start'));
    $detailSession['date_start'] = date('d-m-Y', $date_start);
    
    $date_fin = strtotime($adb->query_result($result, 0, 'due_date'));
    $detailSession['due_date'] = date('d-m-Y', $date_fin);
    
    $detailSession['nbreHeure'] = $adb->query_result($result, 0, 'cf_996');
    $detailSession['nbreJour'] = $adb->query_result($result, 0, 'cf_998');
...
function getListDirection($module, $focus) {
    global $adb;
    $listDirectionBySession = [];
    $query = 'SELECT DISTINCT vtiger_contactscf.cf_1320
        from vtiger_sessionsapprenantsrel 
        INNER JOIN vtiger_contactscf on vtiger_contactscf.contactid = vtiger_sessionsapprenantsrel.apprenantid
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        $listDirectionBySession[$i] = $adb->query_result($result, $i, 'cf_1320');
    }
    return $listDirectionBySession;
}
/* uni_cnfsecrm - v2 - modif 176 - FIN */
