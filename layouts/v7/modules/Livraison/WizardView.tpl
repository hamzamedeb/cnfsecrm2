...
{strip}
    ...
    <form id="EditView" method="POST" enctype="multipart/form-data">
        ...
        <div class="row-fluid">
            ...
            <div class="span8 well"> 
                <div class="textAlignCenter alert alert-info" style="height: 50px;"> <h3 style="margin-top: 5px;">Assistant Livraison </h3></div>
                <hr/>
                <div class="row-fluid">
                    <h4>Nom de livraison</h4><input type="text" name="nom" maxlength="35" class="smallInputBox inputElement validate[required]" value="Livraison" style="width: 200px">
                </div>                                    
                <div class="span2">&nbsp;</div>
            </div>
	...
	<table class="table table-bordered lineItemTableArticle" id="lineItemTabArticle">
            ...
            <tr id="rowArticle0" class="hide lineItemCloneCopyArticle" data-row-num="0">
                    {include file="partials/LineItemsContentArticles.tpl"|@vtemplate_path:'Livraison' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
            </tr>
            {foreach key=row_no item=data from=$RELATED_SESSION_APPRENANTS}
                    <tr id="rowArticle{$row_no}" data-row-num="{$row_no}" class="lineItemRowArticle">
                            {include file="partials/LineItemsContentArticles.tpl"|@vtemplate_path:'Livraison' row_no=$row_no data=$data}
                    </tr>
            {/foreach}
            {if count($RELATED_SESSION_APPRENANTS) eq 0}
                    <tr id="rowArticle1" class="lineItemRowArticle" data-row-num="1">
                            {include file="partials/LineItemsContentArticles.tpl"|@vtemplate_path:'Livraison' row_no=1 data=[] IGNORE_UI_REGISTRATION=false}
                    </tr>
            {/if}
        </table>	
        </div>	        	
    ...
    <div class="btn-toolbar">
        <span class="btn-group">
            ...
            <button type="submit" class="btn btn-primary" id="Btn_Process">{vtranslate('LBL_PROCED_TO_ENVOI', $MODULE )}</button>   
        </div>
    </form>
</div>
</div>
