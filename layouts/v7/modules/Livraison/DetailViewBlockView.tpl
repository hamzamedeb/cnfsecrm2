{strip}
						...
						<tr>
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} {assign var=COUNTER value=$COUNTER+1} {/if}>
	...
	{/foreach}
        ...
        <div class="block block_detailHistoriqueId" data-block="detailHistorylivraisonId" data-blockid="detailHistorylivraisonId">
			<div>
				<h4 class="textOverflowEllipsis maxWidth50">
					<img class="cursorPointer alignMiddle blockToggle {if !($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowRight.png')}" data-mode="hide" data-id=detailHistorylivraisonId>
					<img class="cursorPointer alignMiddle blockToggle {if ($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowdown.png')}" data-mode="show" data-id=detailHistorylivraisonId>&nbsp;
					Historique
				</h4>
			</div>
			<hr>
			<div class="blockData">                            
                            <table style="width: 100%;text-align: center;" class="table-bordered detailview-table">
                                <tbody>
                                    <tr>
                                        <th style="height: 30px; text-align: center;">Produit</th>                                        
                                        <th style="height: 30px; text-align: center;">Quantité avant</th>
                                        <th style="height: 30px; text-align: center;">Quantité après</th>                                        
                                    </tr>
                                </tbody>
                                    <tbody>
                                        {foreach key=BLOCK_LABEL_KEY item=FIELD from=$DETAIL_HISTORIQUE}
                                        <tr style="height: 30px; text-align: center;">
                                            <td><a href="index.php?module=Products&view=Detail&record={$FIELD['productid']}&app=SALES">{$FIELD['productname']}</a></td>                                                                                        
                                            <td>{$FIELD['oldqty']}</td>
                                            <td>{$FIELD['newqty']}</td>
                                        </tr>
                                        {/foreach}
