...
* All Rights Reserved.
*************************************************************************************}
{* uni_cnfsecrm - v2 - modif 113 - FILE *}
{strip}
						...
						<tr>
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} {* uni_cnfsecrm - affichage adresses colspan="3"*} {assign var=COUNTER value=$COUNTER+1} {/if}>
	...
	{/foreach}
        ...
        <div class="block block_detailHistoriqueId" data-block="detailHistoryimpayeeId" data-blockid="detailHistoryimpayeeId">
			<div>
				<h4 class="textOverflowEllipsis maxWidth50">
					<img class="cursorPointer alignMiddle blockToggle {if !($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowRight.png')}" data-mode="hide" data-id=detailHistoryimpayeeId>
					<img class="cursorPointer alignMiddle blockToggle {if ($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowdown.png')}" data-mode="show" data-id=detailHistoryimpayeeId>&nbsp;
					Historique
				</h4>
			</div>
			<hr>
			<div class="blockData">
                            ...
                            <table style="width: 100%;text-align: center;" class="table-bordered detailview-table">
                                <tbody>
                                    <tr>
                                        <th style="height: 30px; text-align: center;">Réponse Par</th>
                                        <th style="height: 30px; text-align: center;">Type Relance</th>
                                        <th style="height: 30px; text-align: center;">Date</th>
                                        <th style="height: 30px; text-align: center;">Date Echéance</th>
                                        <th style="height: 30px; text-align: center;">Commentaire</th>
                                    </tr>
                                </tbody>
                                    <tbody>
                                        {foreach key=BLOCK_LABEL_KEY item=FIELD from=$DETAIL_HISTORIQUE}
                                        <tr style="height: 30px; text-align: center;">
                                            <td>{$FIELD['reponse_par']}</td>
                                            <td>
                                                {if $FIELD['type_relance'] == "Depasse de 7 jours"}
                                                    Dépassé de 7 jours
                                                {else if $FIELD['type_relance'] == "Depasse de 14 jours"}
                                                    Dépassé de 14 jours
                                                {else if $FIELD['type_relance'] == "Depasse de 30 jours"}
                                                    Dépassé de 30 jours
                                                {/if} 
                                            </td>
                                            <td>{$FIELD['date_rappel']}</td>
                                            <td>{$FIELD['date_echeance']}</td>
                                            <td>{$FIELD['commentaire']}</td>
                                        </tr>
                                        {/foreach}
                                    </tbody>
				</table>
