...
jQuery.Class("Emails_MassEdit_Js", {}, {
    ...
    marquerRelance: function (e) {
        var self = this;
        var postData = {};
        var form = jQuery(e.currentTarget).closest('form');
        var formData = new FormData(form[0]);
        var aDeferred = jQuery.Deferred();
        var recordId = $("#current_recordid").val();
        console.log(recordId);
        var etat_echeance = $("#etat_echeance").val();
        console.log(etat_echeance);
        postData['action'] = 'marquerRelance';
        ...
        var dataUrl = "index.php?module=Invoice&action=marquerRelance&record=" + recordId + "&etat_echeance=" + etat_echeance;
        console.log(dataUrl);
        app.request.post({'url': dataUrl}).then(
                function (error, data) {
                    if (data['reponse'] == 'ok') {
                        console.log("relance marqué");
                    }
                },
                );
    },
    ...
    registerEventForRemoveCustomAttachments: function () {
        ...
        jQuery('[name="removeAttachment"]').on('click', function (e) {
            ...
            jQuery.each(attachmentsInfo, function (index, attachmentObject) {
                console.log(attachmentObject.fileid);
    ...
    registerEventForRemoveCustomAttachmentsQuotes: function () {
        var thisInstance = this;
        var composeEmailForm = this.getMassEmailForm();
        jQuery('[name="removeAttachmentQuotes"]').on('click', function (e) {
            var attachmentsContainer = composeEmailForm.find('[ name="attachments"]');
            var attachmentsInfo = JSON.parse(attachmentsContainer.val());
            var element = jQuery(e.currentTarget);
            var imageContainer = element.closest('div.MultiFile-label');
            ...
            jQuery.each(attachmentsInfo, function (index, attachmentObject) {
                if ((typeof attachmentObject != "undefined") && (attachmentObject.id == fileId)) {
                    attachmentsInfo.splice(index, 1);
                }
            });
            attachmentsContainer.val(JSON.stringify(attachmentsInfo));
    ...
    registerEvents: function () {
        ...
        if (container.length > 0) {
            ...
            //unicnfsecrm_mod_18
            if ($('#modepayer').val() == 1) {
                this.registerSaveDraftOrSendEmailEventPayer();
            } else {
            ...
            /* unicnfsecrm_022020_07 */
            this.registerEventForRemoveCustomAttachmentsQuotes();
    ...
    //unicnfsecrm_mod_18
    registerSaveDraftOrSendEmailEventPayer: function () {
        console.log('test payer 1');
        var thisInstance = this;
        var form = this.getMassEmailForm();
        console.log('form');
        console.log(form);
        form.on('click', '#sendEmailPayer, #saveDraftPayer', function (e) {
