...
{strip}
    ...
    {* uni_cnfsecrm *}
    {assign var="tarif" value="tarif"|cat:$row_no}
    {assign var="nbr_jours" value="nbr_jours"|cat:$row_no}
    {assign var="nbr_heures" value="nbr_heures"|cat:$row_no}  
    {assign var="base_heure" value="base_heure"|cat:$row_no}
    {assign var="nature_calcul" value="nature_calcul"|cat:$row_no}  
    {assign var="par_personne" value="par_personne"|cat:$row_no}   
    {assign var="listpriceinter" value="listpriceinter"|cat:$row_no}   
    {assign var="listpriceintra" value="listpriceintra"|cat:$row_no}       
					...
					<div class="col-lg-10">
						<div class="input-group" style="width:100%">
							...
							<input type="hidden" id="lineItemType{$row_no}" name="lineItemType{$row_no}" value="{$entityType}" class="lineItemType"/>
                                                        <input type="hidden" id="{$listpriceinter}" name="{$listpriceinter}" value="{$data.$listpriceinter}" class="listpriceinter"/>
                                                        <input type="hidden" id="{$listpriceintra}" name="{$listpriceintra}" value="{$data.$listpriceintra}" class="listpriceintra"/>
							<div class="col-lg-2">
								{if $row_no eq 0}
	...
	{/if}
        <td class="td_select_inv">
            <select class="tarif select-detail_inv" id="{$tarif}" name="{$tarif}">                                
                    <option value="inter" {if $data.$tarif eq 'inter'}selected{/if}>{vtranslate('Inter', $MODULE)}</option>                  
                    <option value="intra" {if $data.$tarif eq 'intra'}selected{/if}>{vtranslate('Intra', $MODULE)}</option>                                                    
            </select><br/><br/>
            Base <input id="{$base_heure}" name="{$base_heure}" type="text" class="base_heure input-detail_inv inputElement"
			   data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$base_heure)}{$data.$base_heure}{else}7{/if}"/> H
	</td>
        <td class="td_detail_inv">
    		<input id="{$nbr_jours}" name="{$nbr_jours}" type="text" class="nbr_jours input-detail_inv inputElement"
			   data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$nbr_jours)}{$data.$nbr_jours}{else}1{/if}"	/>
	</td>
        <td class="td_detail_inv">
    		<input id="{$nbr_heures}" name="{$nbr_heures}" type="text" class="nbr_heures input-detail_inv smallInputBox inputElement"
			   data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$nbr_heures)}{$data.$nbr_heures}{else}1{/if}"	/>
	</td>
	<td>
		<input id="{$qty}" name="{$qty}" type="text" class="qty input-detail_inv smallInputBox inputElement"
		...
		<td>
			...
			<div style="clear:both"></div><br/>
                        <div style="width:150px;">
                             <td class="td_select_inv">
            <select class="nature_calcul select-detail_inv" id="{$nature_calcul}" name="{$nature_calcul}">                                                         
                                <option value="jour" {if $data.$nature_calcul eq 'jour'}selected{/if}>{vtranslate('Jours', $MODULE)}</option>                  
                                <option value="heure" {if $data.$nature_calcul eq 'heure'}selected{/if}>{vtranslate('Heure', $MODULE)}</option>                  
                                <option value="forfait" {if $data.$nature_calcul eq 'forfait'}selected{/if}>{vtranslate('Forfait', $MODULE)}</option>                  
                            </select>
                        </div><br/>
                        <div style="width:150px;">                            
                            <input id="{$par_personne}" name="{$par_personne}" class="par_personne inputElement" style="width:15px;height:15px;" type="checkbox" {if !empty($data.$par_personne)}checked{/if}> Par Personne
                        </div>
	...
	<td>
		...
		{if $ITEM_DISCOUNT_AMOUNT_EDITABLE || $ITEM_DISCOUNT_PERCENT_EDITABLE}
			<div id="discountTotal{$row_no}" align="right" class="discountTotal hide">{if $data.$discountTotal}{$data.$discountTotal}{else}0{/if}</div>
			<div id="totalAfterDiscount{$row_no}" align="right" class="totalAfterDiscount hide">{if $data.$totalAfterDiscount}{$data.$totalAfterDiscount}{else}0{/if}</div>
		{/if}
