...
{/if}
{*{$LINEITEM_FIELDS['hdnS_H_Percent']|print_r}*}
...
<input type="hidden" name="cnverfa" id="cnverfa" value="{$smarty.get.cnverfa}" />
<input type="hidden" name="isAvoir" id="isAvoir" value="{$smarty.get.isAvoir}" />
{* uni_cnfsecrm - v2 - modif 171 - DEBUT *}
<input type="hidden" name="retro" id="retro" value="{$smarty.get.retro}" />
{* uni_cnfsecrm - v2 - modif 171 - FIN *}
<input type="hidden" name="numfacture_initial" id="numfacture_initial" value="{$smarty.get.record}" />
{*test {$SH_PERCENT_EDITABLE} | {$LINEITEM_FIELDS['hdnS_H_Percent']|print_r} test*}
	...
	{if $BLOCK_FIELDS|@count gt 0}
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab">
					<tr>
						...
						{/if}
                                                <td>
							<strong>{vtranslate('LBL_TARIF',$MODULE)}</strong>
						</td>
                                                <td>
							<strong>{vtranslate('LBL_NBR_JOURS',$MODULE)}</strong>
						</td>
                                                <td>
							<strong>{vtranslate('LBL_NBR_HEURES',$MODULE)}</strong>
						</td>
						<td>
		...
		<div class="fieldBlockContainer">
			<table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
					...
					<tr valign="top">
						<td width="83%" >
							<div class="pull-right">
								<strong>{vtranslate('LBL_FINANCEMENT',$MODULE)}</strong>
							</div>
						</td>
						<td>
                                                        <span class="pull-right"><input id="financement" name="financement" type="text" class="lineItemInputBox form-control" value="{if !empty($FINAL.financement)}{$FINAL.financement}{else}0{/if}"></span>
						</td>
					</tr>
                                        <tr valign="top" class="hide">
						<td width="83%" >
							<div class="pull-right">
								<strong>{vtranslate('LBL_MONTANTCLIENT',$MODULE)}</strong>
							</div>
						</td>
						<td>
                                                        <span class="pull-right"><input id="montantclient" name="montantclient" type="text" class="lineItemInputBox form-control" value="{if !empty($FINAL.montantclient)}{$FINAL.montantclient}{else}0{/if}"></span>
						</td>
					</tr>
                                {/if}
				...
				{if $SH_PERCENT_EDITABLE or $MODULE eq 'Quotes'}
