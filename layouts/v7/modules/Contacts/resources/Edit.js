...
Vtiger_Edit_Js("Contacts_Edit_Js", {}, {
    ...
    registerCityAutoComplete: function (container) {
        console.log('test cp');
        $("input[name='mailingzip']").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: "https://api-adresse.data.gouv.fr/search/?postcode=" + $("input[name='mailingzip']").val(),
                    data: {q: request.term},
                    dataType: "json",
            ...
            select: function (event, ui) {
                ...
                etat = etat.split(', ')
                var region = etat[2];
                $("input[name='mailingcity']").val(ui.item.city);
                $("input[name='mailingstate']").val(region);
            }
        });
    },
