...
* All Rights Reserved.
************************************************************************************}
{*uni_cnfsecrm - v2 - modif 169 - FILE*}
{strip}
					...
					{foreach item=RELATED_RECORD from=$RELATED_RECORDS}
						<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
							{if $RELATED_MODULE_NAME eq 'Calendar'}
								...
								{/if}
							{else}
                                                            {* uni_cnfsecrm - v2 - modif 169 - DEBUT *}
                                                            {if $RELATED_MODULE_NAME neq 'Invoice'}
								data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'
                                                            {/if}  
                                                            {* uni_cnfsecrm - v2 - modif 169 - FIN *}
							{/if}>
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								<td class="relatedListEntryValues" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}"  
                                                                    {* uni_cnfsecrm - v2 - modif 169 - DEBUT *}
                                                                        {if $RELATED_MODULE_NAME eq 'Invoice'}
                                                                            onclick="window.location.href = '{$RELATED_RECORD->getDetailViewUrl()}'"
                                                                        {/if} 
                                                                    {* uni_cnfsecrm - v2 - modif 169 - FIN *}
                                                                    nowrap
                                                                    >
