...
Vtiger_Edit_Js("HistoryRecyclage_Edit_Js", {
    ...
    addEditRow: function (typeAction, id) {
        ...
        console.log(reponsePar, commentaire, dateRappel, reponse, typeAction)
        var apprenantId = $('.contactid').val();
        var sessionId = $('.activityid').val();
        var recordId = $('.recordId').val();

        if (reponse == 0) {
            var messageResultErreur = "Vous devez choisir la réponse de l'apprenant !";
        ...
        var messageResult;
        var dataUrl = "index.php?module=HistoryRecyclage&action=ajoutHistorique&record=" + recordId + "&reponsePar=" + reponsePar + "&commentaire=" + commentaire + "&dateRappel=" + dateRappel + "&reponse=" + reponse + "&apprenantId=" + apprenantId + "&sessionId=" + sessionId + "&etreRappeler=" + etreRappeler + "&id=" + id + "&typeAction=" + typeAction + "&updateModule=" + updateModule;
    ...
    createNewRow: function (data, lastrow, type) {
        ...
        row += " <td><span onclick='HistoryRecyclage_Edit_Js.editRow(" + data['id'] + ")' class='fa fa-pencil'></span></td>";
        row += " </tr>";
        ...
        row += "        <select onchange='HistoryRecyclage_Edit_Js.selectReponse(event, " + data['id'] + ")' class='form-control reponse'>"
        row += "            <option "
        if (data['reponseId'] == 0) {
            row += "           selected='selected' "
        }
        ...
        row += "    </td>";
        row += " <td> <span onclick='HistoryRecyclage_Edit_Js.addEditRow(\"edit\", " + data['id'] + ")' class='pointerCursorOnHover input-group-addon input-group-addon-save'><i class='fa fa-check'></i></span> </td>"
        row += " <td> <span onclick='HistoryRecyclage_Edit_Js.annulerEditRow(" + data['id'] + ")' class='pointerCursorOnHover input-group-addon input-group-addon-cancel'><i class='fa fa-close'></i></span> </td>"
        row += " </tr> ";
        if (type == "save") {
