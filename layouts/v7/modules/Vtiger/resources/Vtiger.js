...
Vtiger.Class('Vtiger_Index_Js', {
    ...
    registerActivityReminder: function () {
        console.log("registerActivityReminder");
        ...
        if (activityReminderInterval != '') {
            ...
            if (currentTime >= nextActivityReminderCheck) {
                ...
                /* uni_cnfsecrm - v2 - modif 130 - DEBUT */
                Vtiger_Index_Js.requestReminderRecyclage();
                Vtiger_Index_Js.requestReminderSansSession();
                Vtiger_Index_Js.requestReminderSuiviProspects();
                Vtiger_Index_Js.requestReminderHistoryImpayes();
...
//		} 
     ...
     * Function request for reminder popups
     */
    requestReminderRecyclage: function () {
        /* uni_cnfsecrm - reminder recyclage */
        var activityReminder = app.getActivityReminderInterval();
//		if(!activityReminder) {
//			return;
//		} 
        ...
        app.request.post({
            'data': {
                'module': 'Calendar',
                'action': 'ActivityReminderRecyclage',
                'mode': 'getReminders'
            }
        }).then(function (e, res) {
            if (!res.hasOwnProperty('result')) {
                for (i = 0; i < res.length; i++) {
                    var record = res[i];
                    if (typeof record == 'object') {
                        Vtiger_Index_Js.showReminderPopupRecyclage(record);
                    }
                }
            }
        });
    ...
    requestReminderSansSession: function () {
        /* uni_cnfsecrm - reminder recyclage */
        var activityReminder = app.getActivityReminderInterval();
//		if(!activityReminder) {
//			return;
//		} 
        ...
        app.request.post({
            'data': {
                'module': 'Calendar',
                'action': 'ActivityReminderSansSession',
                'mode': 'getReminders'
            }
        }).then(function (e, res) {
            if (!res.hasOwnProperty('result')) {
                for (i = 0; i < res.length; i++) {
                    var record = res[i];
                    if (typeof record == 'object') {
                        Vtiger_Index_Js.showReminderPopupSansSession(record);
                    }
                }
            }
        });
    ...
    requestReminderSuiviProspects: function () {
        /* uni_cnfsecrm - reminder recyclage */
        var activityReminder = app.getActivityReminderInterval();
//		if(!activityReminder) {
//			return;
//		} 
        ...
        app.storage.set('activityReminder', 1); 
        //setting next activity reminder check time
        app.storage.set('nextActivityReminderCheckTime', currentTime + parseInt(activityReminder));
        app.request.post({
            'data': {
                'module': 'Calendar',
                'action': 'ActivityReminderSuiviProspects',
                'mode': 'getReminders'
            }
        }).then(function (e, res) {
            if (!res.hasOwnProperty('result')) {
                for (i = 0; i < res.length; i++) {
                    var record = res[i];
                    if (typeof record == 'object') {
                        Vtiger_Index_Js.showReminderPopupSuiviProspects(record);
                    }
                }
            }
        });
    ...
    requestReminderHistoryImpayes: function () {
        /* uni_cnfsecrm - reminder recyclage */
        console.log('test 01')
        var activityReminder = app.getActivityReminderInterval();
//		if(!activityReminder) {
...
//		} 
        ...
        app.request.post({
            'data': {
                'module': 'Calendar',
                'action': 'ActivityReminderHistoryImpayes',
                'mode': 'getReminders'
            }
        }).then(function (e, res) {
            if (!res.hasOwnProperty('result')) {
                for (i = 0; i < res.length; i++) {
                    ...
                    if (typeof record == 'object') {
                        console.log('test 04')
                        Vtiger_Index_Js.showReminderPopupHistoryImpayes(record);
                    }
                }
            }
        });
     ...
     * Function display the Reminder popup
     */
    showReminderPopupRecyclage: function (record) {
        var notifyParams = {
            /* uni_cnfsecrm - v2 - modif 135 - DEBUT */
            'title': '<a style="font-size: 15px;" target="_blank" href="index.php?module=HistoryRecyclage&parent=&page=1&view=List&viewname=102&orderby=&sortorder=&app=TOOLS&search_params=%5B%5B%5B%22name%22%2C%22c%22%2C%22' + record.name + '%22%5D%5D%5D&tag_params=%5B%5D&nolistcache=0&list_headers=%5B%22name%22%2C%22">' + record.name + '</a>&nbsp;&nbsp;' +
                    '<i id="reminder-postpone-' + record.record_id + '" title="' + app.vtranslate('JS_POSTPONE') + '" class="cursorPointer fa fa-clock-o"></i>',
            /* uni_cnfsecrm - v2 - modif 135 - FIN */
            'message': '<div class="col-sm-12">' +
                    '<div class="row">' +
                    '<div class="col-sm-12 font13px">' +
                    'Date de Rappel : ' + record.cf_1256 +
                    '</div>' +
                    '</div>' +
                    '</div>'
        };
        ...
        jQuery.notify(notifyParams, settings);
        jQuery('#reminder-postpone-' + record.record_id).on('click', function (e) {
            jQuery(e.currentTarget).closest('.notificationHeader').find('[data-notify="dismiss"]').trigger('click');
            app.request.post({
                'data': {
                    'module': 'Calendar',
        ...
        jQuery('#reminder-postpone-' + record.record_id).closest('[data-notify="container"]').draggable({'containment': 'body'});
    },
    
    showReminderPopupSansSession: function (record) {
        var notifyParams = {
            /* uni_cnfsecrm - v2 - modif 135 - DEBUT */
            'title': '<a style="font-size: 15px;" target="_blank" href="index.php?module=HistorySansSessions&parent=&page=1&view=List&viewname=104&orderby=&sortorder=&app=TOOLS&search_params=[[[%22name%22,%22c%22,%22' + record.name + '%22]]]">' + record.name + '</a>&nbsp;&nbsp;' +
                    '<i id="reminder-postpone-' + record.record_id + '" title="' + app.vtranslate('JS_POSTPONE') + '" class="cursorPointer fa fa-clock-o"></i>',
            /* uni_cnfsecrm - v2 - modif 135 - FIN */
            'message': '<div class="col-sm-12">' +
                    '<div class="row">' +
                    '<div class="col-sm-12 font13px">' +
                    'Date de Rappel : ' + record.cf_1283 +
                    '</div>' +
                    '</div>' +
                    '</div>'
        };
    ...
    showReminderPopupSuiviProspects: function (record) {
        var notifyParams = {
            /* uni_cnfsecrm - v2 - modif 135 - DEBUT */
            'title': '<a style="font-size: 15px;" target="_blank" href="index.php?module=SuiviProspects&parent=&page=1&view=List&viewname=105&orderby=&sortorder=&app=TOOLS&search_params=[[[%22name%22%2C%22c%22%2C%22' + record.name + '%22]]]">' + record.name + '</a>&nbsp;&nbsp;' +
                    '<i id="reminder-postpone-' + record.record_id + '" title="' + app.vtranslate('JS_POSTPONE') + '" class="cursorPointer fa fa-clock-o"></i>',
            /* uni_cnfsecrm - v2 - modif 135 - FIN */
            'message': '<div class="col-sm-12">' +
                    '<div class="row">' +
                    '<div class="col-sm-12 font13px">' +
                    'Date de Rappel : ' + record.cf_1295 +
                    '</div>' +
                    '</div>' +
                    '</div>'
        };
    ...
    showReminderPopupHistoryImpayes: function (record) {
        var notifyParams = {
            /*uni_cnfsecrm - v2 - modif 135 - DEBUT*/
            'title': '<a style="font-size: 15px;" target="_blank" href="index.php?module=HistoryImpayes&parent=&page=1&view=List&viewname=103&orderby=&sortorder=&app=TOOLS&search_params=[[[%22name%22%2C%22c%22%2C%22' + record.name + '%22]]]">' + record.name + '</a>&nbsp;&nbsp;' +
                    '<i id="reminder-postpone-' + record.record_id + '" title="' + app.vtranslate('JS_POSTPONE') + '" class="cursorPointer fa fa-clock-o"></i>',
            /* uni_cnfsecrm - v2 - modif 135 - FIN */
            'message': '<div class="col-sm-12">' +
                    '<div class="row">' +
                    '<div class="col-sm-12 font13px">' +
                    'Date de Rappel : ' + record.cf_1270 +
                    '</div>' +
                    '</div>' +
                    '</div>'
        };
    ...
    sendEmailPDFClickHandlerPayer: function (url) {
        var self = this;
        ...
        app.request.post({data: params}).then(function (err, response) {
            var callback = function () {
                ...
                $('#sendEmailPayer').on('click', function () {
                    self.marquerRappeler();
                })
            };
    ...
    marquerRappeler: function () {
        var invoice = $('#current_recordid').val();
        var dataUrl = "index.php?module=HistoryImpayes&action=MarquerRappeler&record=" + invoice;
        app.request.get({'url': dataUrl}).then(
                function (error, data) {
                    console.log(data)
                    if (error == null) {
                        $('#' + invoice).parent().parent().remove();
                        if ($('#detailHistory div').hasClass('btnDetailHistory') == false)
                        {
                            var btn = '';
                            btn += "<div class='btn-group pull-left btnDetailHistory' style='padding-left: 20px'>";
                            btn += "<a href='index.php?module=HistoryImpayes&view=Detail&record=" + data['idHistoryImpayes'] + "&app=TOOLS'target='_blank' class='btn btn-success btn-xs'>";
                            btn += "Afficher Historique Impayée";
                            btn += "</a>";
                            btn += "</div>";
                            $('#detailHistory').prepend(btn);
                        }
                    }
                },
                function (error, err) {
    ...
    registerAutoCompleteFields: function (container) {
        ...
        container.find('input.autoComplete').autocomplete({
            ...
            'select': function (event, ui) {
                ...
                /*uni_cnfsecrm - v2 - modif 137 - DEBUT*/
                thisInstance.setNomSessionAuto(selectedItemData.id);
                /*uni_cnfsecrm - v2 - modif 137 - FIN*/
...
//					
    ...
    registerNavigationEvents: function (container, recordId) {
        //registerNavigationEvents: function (container) { /*unicnfsecrm_gestimpaye_01 - ajout recordId */
        ...
        this.detailViewSaveComment(container, recordId);
        this.editComment(container);
        this.replyComment(container);
        this.detailViewThread(container);
        this.closeCommentBlock(container);
        this.moreRecentComments(container);
        this.registerMultiUpload();

        //fin unicnfsecrm_gestimpaye_01 : commentaires

        // unicnfsecrm_gestimpaye_02 : envoi mail relance
        this.registerSaveDraftOrSendEmailEvent(container);
        // fin unicnfsecrm_gestimpaye_02 : envoi mail relance

        // unicnfsecrm_gestimpaye_04 : Etat échéance
        this.updateEtatEcheance(container, recordId);
        // fin unicnfsecrm_gestimpaye_04 : Etat échéance

        // unicnfsecrm_mod_19
        this.updateNePasRelancer(container, recordId);
        //fin unicnfsecrm_mod_19
        //this.testFilter(container);

    },

    /* testFilter: function (container) {
     console.log('test')
    ...
    registerNextRecordClickEvent: function (container) {
        ...
        container.find('#quickPreviewNextRecordButton').on('click', function (e) {
            ...
            //self.showQuickPreviewForId(nextRecordId, moduleName, appName, templateId, false, 'navigation'); /* avant modif */
            var filtertab = $('.nomfilter').val();
            if (filtertab != '' || filtertab != 'undefined') {
                self.showQuickPreviewForIdFilter(filtertab, nextRecordId, moduleName, appName, templateId, false, 'navigation');
            } else {
    ...
    registerPreviousRecordClickEvent: function (container) {
        ...
        container.find('#quickPreviewPreviousRecordButton').on('click', function (e) {
            ...
            //unicnfsecrm_mod_15 ( quickView )
            //self.showQuickPreviewForId(prevRecordId, moduleName, appName, templateId, false, 'navigation'); /* comment after modif */
            var filtertab = $('.nomfilter').val();
            if (filtertab != '' || filtertab != 'undefined') {
                self.showQuickPreviewForIdFilter(filtertab, prevRecordId, moduleName, appName, templateId, false, 'navigation');
            } else {
        ...
        app.request.get({data: params}).then(function (err, response) {
            ...
            var params = {
                cb: function () {
                    ...
                    //unicnfsecrm_gestimpaye_01 : commentaires (ajouter recordid)
                    //thisInstance.registerNavigationEvents(jQuery('#helpPageOverlay'));
                    thisInstance.registerNavigationEvents(jQuery('.quickPreview'), recordId);
                    //fin unicnfsecrm_gestimpaye_01 : commentaires 
                }
            };
    ...
    showQuickPreviewForId: function (recordId, moduleName, appName, templateId, isReference, mode) {
        ...
        /* uni_cnfsecrm - modif 102 - DEBUT */
        /* uni_cnfsecrm - v2 - modif 103 - DEBUT */
        /* uni_cnfsecrm - v2 - modif 107 - DEBUT */
        /* uni_cnfsecrm - v2 - modif 120 - DEBUT */
        else if (moduleName == 'HistoryRecyclage' || moduleName == 'HistoryImpayes' || moduleName == 'HistorySansSessions' || moduleName == 'SuiviProspects') {
            self._showHistoryRecyclageQuickPreviewForId(recordId, moduleName, templateId, isReference, mode);
        }
        /* uni_cnfsecrm - v2 - modif 120 - FIN */
        /* uni_cnfsecrm - v2 - modif 107 - FIN */
        /* uni_cnfsecrm - modif 102 - FIN */
        /* uni_cnfsecrm - v2 - modif 103 - FIN */
        else {
    ...
    /* uni_cnfsecrm - modif 102 - DEBUT */
    _showHistoryRecyclageQuickPreviewForId: function (recordId, moduleName, templateId, isReference, mode) {
        var thisInstance = this;
        var params = {};
        if (typeof moduleName === 'undefined') {
            moduleName = app.module();
    ...
    detailViewSaveComment: function (container, recordId) {
        var self = this;
        container.on('click', '.detailViewSaveComment', function (e) {
            var element = jQuery(e.currentTarget);
            if (!element.is(":disabled")) {
                var dataObj = self.saveComment(e, recordId);
                dataObj.then(function () {
                    var commentsContainer = container.find("[data-name='ModComments']");
    ...
    loadWidgets: function (container) {
        var self = this;
        var widgetList = container.find('.widgetContainer_comments');
     ...
     */
    saveComment: function (e, recordId) {
        var self = this;
        var aDeferred = jQuery.Deferred();
        ...
        var a = self.getRecordId(recordId);
        a = a.toString()
        if (!commentRelatedTo)
            commentRelatedTo = a;
...
//bouton d'edit
    editComment: function (container) {
        var self = this;
        container.on('click', '.editComment', function (e) {
    ...
    removeCommentBlockIfExists: function () {
        var Commentswidget = jQuery('.commentsBody');
        jQuery('.addCommentBlock', Commentswidget).remove();
    },

    getEditCommentBlock: function () {
        var clonedCommentBlock = jQuery('.basicEditCommentBlock').clone(true, true).removeClass('basicEditCommentBlock hide').addClass('addCommentBlock');
    ...
    //fin bouton d'edit
    //bouton repondre
    replyComment: function (container) {
        var self = this;
        container.on('click', '.replyComment', function (e) {
            self.removeCommentBlockIfExists();
            var currentTarget = jQuery(e.currentTarget);
            var commentInfoBlock = currentTarget.closest('.singleComment');
    ...
    getCommentBlock: function () {
        var clonedCommentBlock = jQuery('.basicAddCommentBlock').clone(true, true).removeClass('basicAddCommentBlock hide').addClass('addCommentBlock');
        clonedCommentBlock.find('.commentcontenthidden').removeClass('commentcontenthidden').addClass('commentcontent');
        return clonedCommentBlock;
    },

    detailViewThread: function (container) {
        var self = this;
        container.find('.detailViewThread').on('click', function (e) {
    ...
    moreRecentComments: function (container) {
        var self = this;
        container.find('.moreRecentComments').on('click', function (e) {
            var recentCommentsTab = self.getTabByLabel(self.detailViewRecentCommentsTabLabel);
    ...
    //fermer commentaire block
    closeCommentBlock: function (container) {
        var self = this;
        container.on('click', '.closeCommentBlock', function (e) {
            var currentTarget = jQuery(e.currentTarget);
            var commentInfoBlock = currentTarget.closest('.singleComment');
    ...
    // unicnfsecrm_gestimpaye_02 : envoi mail relance
    registerSaveDraftOrSendEmailEvent: function (container) {
        var thisInstance = this;
        var form = container.find('#massEmailForm');

        var toemail = $('#massEmailForm #toEmail').val();
                ...
                submitHandler: function (form) {
                    ...
                    app.request.post(postParams).then(function (err, data) {
                        ...
                        if ((toemail == '') || (toemail == 'undefined')) {
                            data = "<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><div class='clearfix'><div class='pull-right ' ><button type='button' class='close' aria-label='Close' data-dismiss='modal'><span aria-hidden='true' class='fa fa-close'></span></button></div><h4 class='pull-left'>Result</h4></div></div><div class='modal-body'><div class='mailSentSuccessfully' data-relatedload=''>echec d'envoie email introuvable </div><input type='hidden' id='flag' value='SENT'></div></div></div>";
                        } else {
                            data = "<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><div class='clearfix'><div class='pull-right ' ><button type='button' class='close' aria-label='Close' data-dismiss='modal'><span aria-hidden='true' class='fa fa-close'></span></button></div><h4 class='pull-left'>Result</h4></div></div><div class='modal-body'><div class='mailSentSuccessfully' data-relatedload=''>email de relance envoyer avec succès </div><input type='hidden' id='flag' value='SENT'></div></div></div>";
                        }
                        app.helper.showModal(data);
                    });
                }
    ...
    // unicnfsecrm_gestimpaye_04 : Etat échéance
    updateEtatEcheance: function (container, recordId) {
        var self = this;
        container.find('.updateetatecheance').on('click', function (e) {
            var postData = {};
            var form = jQuery(e.currentTarget).closest('form');
            var formData = new FormData(form[0]);
            var aDeferred = jQuery.Deferred();
            postData['action'] = 'EtatEcheance';
            postData['module'] = 'Invoice';
            ...
            var dataUrl = "index.php?module=Invoice&action=EtatEcheance&record=" + recordId;
            app.request.post({'url': dataUrl}).then(
                    function (error, data) {
                        if (data['reponse'] == 'ok') {
                            var datasuccsess = "<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><div class='clearfix'><div class='pull-right ' ><button type='button' class='close' aria-label='Close' data-dismiss='modal'><span aria-hidden='true' class='fa fa-close'></span></button></div><h4 class='pull-left'>Result</h4></div></div><div class='modal-body'><div class='mailSentSuccessfully' data-relatedload=''>le facture est reporté de 7 jours</div><input type='hidden' id='flag' value='SENT'></div></div></div>";
                        }
    ...
    //unicnfsecrm_mod_15 ( quickView )
    _showInventoryQuickPreviewForIdFilter: function (filtertab, recordId, moduleName, templateId, isReference, mode) {
        var thisInstance = this;
        var params = {};
        if (typeof moduleName === 'undefined') {
            moduleName = app.module();
        ...
        app.request.get({data: params}).then(function (err, response) {
            ...
            var params = {
                cb: function () {
                    ...
                    thisInstance.registerNavigationEvents(jQuery('.quickPreview'), recordId);
                    //ajout le filter
                    jQuery('.quickPreview .nomfilter').val(filtertab);
    ...
    showQuickPreviewForIdFilter: function (filtertab, recordId, moduleName, appName, templateId, isReference, mode) {
        var self = this;
        if (self.isInventoryModule(moduleName)) {
            self._showInventoryQuickPreviewForIdFilter(filtertab, recordId, moduleName, templateId, isReference, mode);
        }
        /* uni_cnfsecrm - modif 102 - DEBUT */
        /* uni_cnfsecrm - v2 - modif 103 - DEBUT */
        /* uni_cnfsecrm - v2 - modif 107 - DEBUT */
    ...
    // unicnfsecrm_mod_19
    updateNePasRelancer: function (container, recordId) {
        var self = this;
        container.find('.NePasRelancer').on('click', function (e) {
            var postData = {};
            var form = jQuery(e.currentTarget).closest('form');
            var formData = new FormData(form[0]);
            var aDeferred = jQuery.Deferred();
            postData['action'] = 'NePasRelancer';
            postData['module'] = 'Invoice';
            postData['record'] = recordId;
            jQuery.each(postData, function (key, value) {
                formData.append(key, value);
            ...
            var dataUrl = "index.php?module=Invoice&action=NePasRelancer&record=" + recordId;
            app.request.post({'url': dataUrl}).then(
                    function (error, data) {
                        if (data['reponse'] == 'ok') {
                            var datasuccsess = "<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><div class='clearfix'><div class='pull-right ' ><button type='button' class='close' aria-label='Close' data-dismiss='modal'><span aria-hidden='true' class='fa fa-close'></span></button></div><h4 class='pull-left'>Result</h4></div></div><div class='modal-body'><div class='mailSentSuccessfully' data-relatedload=''>La facture est modifié : A ne pas relancer</div><input type='hidden' id='flag' value='SENT'></div></div></div>";
                        }
                        app.helper.showModal(datasuccsess);
                        if (error == null) {
    ...
    setNomSessionAuto: function(idFormation){
        var url = "index.php?module=Calendar&action=GetNomFormation&record=" + idFormation;
        app.request.get({'url': url}).then(
                function (error, data) {
                    if (error == null) {
                        console.log(data)
                        $('input[name="subject"]').val(data['nom'])
                    } 
                }
        )
    }
