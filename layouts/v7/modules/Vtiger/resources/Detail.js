...
Vtiger.Class("Vtiger_Detail_Js", {
    ...
    /* uni_cnfsecrm - modif 98 - DEBUT */
    rappelerApprenant: function (apprenantId) {
        var message = "Sûr de marquer l'apprenant comme rappeler";
        var messageResult;
        console.log(apprenantId)
        app.helper.showConfirmationBox({'message': message}).then(
                function (e) {
                    var dataUrl = "index.php?module=Contacts&action=Rappeler&record=" + app.getRecordId() + "&apprenantId=" + apprenantId;
                    app.request.get({'url': dataUrl}).then(
                            function (error, data) {
                                if (error == null) {
                                    console.log(data)
                                    if (data['resultat'] == true) {
                                        $('#row' + apprenantId).remove();
                                    }
    ...
    /* uni_cnfsecrm - modif 98 - FIN */

    /* uni_cnfsecrm - modif 101 - DEBUT */
    /* uni_cnfsecrm - v2 - modif 111 - DEBUT */
    openPopupRecyclage: function (sessionId, apprenantId) {
        var self = this;
        self.showQuickPreviewForIdRecyclage(sessionId, apprenantId);
    },
    showQuickPreviewForIdRecyclage: function (apprenantId, sessionId) {
        var self = this;
        var params = {};
        params['module'] = 'Contacts';
        params['record'] = apprenantId;
        params['sesion'] = sessionId;
        params['view'] = 'PopUpApprenantRecyclageDetail';
        params['navigation'] = 'true';
        /* uni_cnfsecrm - modif 105 - DEBUT */
        params['filterList'] = $('#filterListRecyclage').val();
    ...
    sendEmailPDFClickHandlerRecyclage: function (url) {
        var self = this;
        ...
        app.request.post({data: params}).then(function (err, response) {
            var callback = function () {
                ...
                self.marquerRappeler(apprenantId);
            };
    ...
    marquerRappeler: function (appr) {
        var session = $('.sessionId').val();
        $('#sendEmail').on('click', function () {
            var dataUrl = "index.php?module=Contacts&action=MarquerRappeler&record=" + appr + "&type=marquerRappeler&sessionId=" + session;
            app.request.get({'url': dataUrl}).then(
                    function (error, data) {
                        if (error == null) {
                            $('#rappeler').parent('div').remove();
                            $('#row' + appr).remove();
                            var btn = '';
                            /* uni_cnfsecrm - v2 - modif 131 - DEBUT */
                            btn += "<div class='col-lg-4'>";
                            btn += " <a id='afficherDetail' href='index.php?module=HistoryRecyclage&view=Detail&record=" + data['idHistoryRecyclage'] + "&app=TOOLS'>  <button class='btn btn-success btn-xs'> Afficher Historique Recyclages </button> </a>";
                            /* uni_cnfsecrm - v2 - modif 131 - FIN */
                            btn += "</div>";
                            $('#rowBtn').prepend(btn);
                            $('.RappelerValue').text(1);
                            var messageResult = data['message'];
                            data = "<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><div class='clearfix'><div class='pull-right ' ><button type='button' class='close' aria-label='Close' data-dismiss='modal'><span aria-hidden='true' class='fa fa-close'></span></button></div><h4 class='pull-left'>Result</h4></div></div><div class='modal-body'><div class='mailSentSuccessfully' data-relatedload=''>" + messageResult + "</div><input type='hidden' id='flag' value='SENT'></div></div></div>";
                            app.helper.hideProgress();
                            app.helper.showModal(data);
                        }
    ...
    nePlusRappeler: function (apprenantId) {
        console.log(apprenantId)
        var session = $('.sessionId').val();
        var message = "Sûr de ne plus rappler l'apprenant ";
        var messageResult;
        app.helper.showConfirmationBox({'message': message}).then(
                function (e) {
                    var dataUrl = "index.php?module=Contacts&action=MarquerRappeler&record=" + apprenantId + "&type=nePlusRappeler&sessionId=" + session;
                    app.request.get({'url': dataUrl}).then(
                            function (error, data) {
                                if (error == null) {
                                    var messageResult = data['message'];
                                    ...
                                    app.helper.hideProgress();
                                    app.helper.showModal(data);
                                    $('#nePlusRappeler').attr('disabled', true);
                                    $('#rappeler').attr('disabled', true);
                                    $('#row' + apprenantId).remove();
                                }
                            },
                            function (error, err) {
    ...
    openPopupSansSession: function (apprenantId) {
        var self = this;
        self.showPopUpSansSession(apprenantId);
    },
    showPopUpSansSession: function (apprenantId) {
        var self = this;
        var params = {};
        params['module'] = 'Contacts';
        params['record'] = apprenantId;
        params['view'] = 'PopUpApprenantSansSessionDetail';
        params['navigation'] = 'true';
        params['filterList'] = $('#filterSansSession').val();
        app.helper.showProgress();
        app.request.get({data: params}).then(function (err, response) {
            app.helper.hideProgress();
            jQuery('#helpPageOverlay').css({"width": "550px", "box-shadow": "-8px 0 5px -5px lightgrey", 'height': '100vh', 'background': 'white'});
    ...
    sendEmailSansSession: function (url) {
        var self = this;
        var apprenantId = $('#apprenantId').val();
        console.log('ap ' + apprenantId)
        var params = app.convertUrlToDataParams(url);
        app.helper.showProgress();
        app.request.post({data: params}).then(function (err, response) {
            var callback = function () {
                var emailEditInstance = new Emails_MassEdit_Js();
                emailEditInstance.registerEvents();
                self.marquerRappelerSansSession(apprenantId);
            };
    ...
    marquerRappelerSansSession: function (appr) {
        console.log('app ')
        console.log($('#rowSansSession' + appr))
        $('#sendEmail').on('click', function () {
            var dataUrl = "index.php?module=Contacts&action=RappelerSansSession&record=" + appr + "&type=marquerRappeler";
            app.request.get({'url': dataUrl}).then(
                    function (error, data) {
                        if (error == null) {
                            $('#rappelerSansSession').parent('div').remove();
                            $('#rowSansSession' + appr).remove();
                            var btn = '';
                            /* uni_cnfsecrm - v2 - modif 136 - DEBUT */
                            btn += "<div class='col-lg-5'>";
                            btn += " <a id='afficherDetail' href='index.php?module=HistorySansSessions&view=Detail&record=" + data['idHistorySansSession'] + "&app=TOOLS'>  <button class='btn btn-success btn-xs'> Afficher Historique Sans Sessions </button> </a>";
    ...
    nePlusRappelerSansSession: function (apprenantId) {
        var message = "Sûr de ne plus rappler l'apprenant ";
        var messageResult;
        app.helper.showConfirmationBox({'message': message}).then(
                function (e) {
                    var dataUrl = "index.php?module=Contacts&action=RappelerSansSession&record=" + apprenantId + "&type=nePlusRappeler";
                    app.request.get({'url': dataUrl}).then(
                            function (error, data) {
                                if (error == null) {
                                    var messageResult = data['message'];
                                    ...
                                    app.helper.hideProgress();
                                    app.helper.showModal(data);
                                    $('#nePlusRappelerSansSession').remove();
                                    $('#rappelerSansSession').remove();
                                    $('#rowSansSession' + apprenantId).remove();
                                }
                            },
                            function (error, err) {
    ...
    openPopupProspects: function (accountId, quoteId) {
        var self = this;
        self.showQuickPreviewForIdProspects(accountId, quoteId);
    },
    showQuickPreviewForIdProspects: function (accountId, quoteId) {
        var self = this;
        var params = {};
        params['module'] = 'Contacts';
        params['record'] = accountId;
        params['quoteId'] = quoteId;
        params['view'] = 'PopUpProspectsDetail';
        params['navigation'] = 'true';
        /* uni_cnfsecrm - modif 105 - DEBUT */
        params['filterList'] = $('#filterProspect').val();
        /* uni_cnfsecrm - modif 105 - FIN */
        app.helper.showProgress();
        app.request.get({data: params}).then(function (err, response) {
            app.helper.hideProgress();
    ...
    sendEmailPDFClickHandlerProspects: function (url) {
        var self = this;
        var accountId = $('#idclient').val();
        ...
        app.request.post({data: params}).then(function (err, response) {
            var callback = function () {
                var emailEditInstance = new Emails_MassEdit_Js();
                emailEditInstance.registerEvents();
                self.marquerRappelerProspects(accountId);
            };
            var data = {};
            data['cb'] = callback;
            app.helper.hideProgress();
    ...
    marquerRappelerProspects: function (accountId) {
        var devisId = $('.iddevis').val();
        $('#sendEmail').on('click', function () {
            var dataUrl = "index.php?module=Contacts&action=MarquerRappelerProspects&record=" + accountId + "&type=marquerRappeler&devisId=" + devisId;
            app.request.get({'url': dataUrl}).then(
                    function (error, data) {
                        if (error == null) {
                            $('#rappeler').parent('div').remove();
                            ...
                            /* uni_cnfsecrm - v2 - modif 136 - DEBUT */
                            btn += "<div class='col-lg-4'>";
                            btn += " <a id='afficherDetail' href='index.php?module=SuiviProspects&view=Detail&record=" + data['suiviprospectsid'] + "&app=TOOLS'>  <button class='btn btn-success btn-xs'> Afficher Historique Prospects </button> </a>";
                            /* uni_cnfsecrm - v2 - modif 136 - FIN */
                            btn += "</div>";
                            $('#rowBtn').prepend(btn);
                            $('.RappelerValue').text(1);
    ...
    nePlusRappelerProspects: function (accountId) {
        console.log(accountId)
        var quoteId = $('.iddevis').val();
        var message = "Sûr de ne plus rappler le prospect ";
        var messageResult;
        app.helper.showConfirmationBox({'message': message}).then(
                function (e) {
                    var dataUrl = "index.php?module=Contacts&action=MarquerRappelerProspects&record=" + accountId + "&type=nePlusRappeler&devisId=" + quoteId;
                    app.request.get({'url': dataUrl}).then(
                            function (error, data) {
                                if (error == null) {
                                    var messageResult = data['message'];
    ...
    getDeleteMessageKey: function () {
        console.log("getDeleteMessageKey");
    ...
    saveComment: function (e) {
        ...
        if (!commentRelatedTo)
            commentRelatedTo = self.getRecordId();
    ...
    updateRelatedRecordsCount: function () {
        ...
        var recordId = self.getRecordId();
        var moduleName = app.getModuleName();
        /* unicnfsecrm - problème ajout documents session */
    ...
    registerRelatedRowClickEvent: function () {
        ...
        detailContentsHolder.on('click', '.listViewEntries', function (e) {
            ...
            if (selection.length == 0) {
                ...
                if (targetElement.hasClass('js-reference-display-value'))
                    return;
