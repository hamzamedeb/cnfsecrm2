...
//                params['page'] = this.getCurrentPageNum();
    ...
    addRelations: function (idList) {
        ...
        console.log("relatedModuleName " + relatedModuleName);
        var selectedTabElement = this.getSelectedTabElement();
        console.log("length " + selectedTabElement.length);
        if (selectedTabElement.length > 0) {
            var relationId = selectedTabElement.data('relationId');
            var linkkey = selectedTabElement.data('linkKey');
            console.log("linkkey " + linkkey);
            if (linkkey == "LBL_RECORD_SUMMARY")
            {
                console.log("t01");
                relationId = jQuery('.relationId').val();
    ...
    totalNumOfRecords: function (curEle) {
        ...
        var totalPageNumber = element.text();
        var pageCount;
        if (curEle.attr('id') !== 'relatedViewPageJump')
