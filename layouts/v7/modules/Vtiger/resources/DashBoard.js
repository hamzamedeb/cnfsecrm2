...
Vtiger.Class("Vtiger_DashBoard_Js", {
    ...
    registerEvents: function () {
        ...
        app.event.on("post.DashBoardTab.load", function (event, dashBoardInstance) {
            ...
            instance.registerEventToShowQuickPreview();
            //fin unicnfsecrm_mod_15 ( quickView )
        });
        app.event.trigger("post.DashBoardTab.load");
    },

    showQuickPreviewForIdFilter: function (filtertab, recordId, appName) {
        var self = this;
        var vtigerInstance = Vtiger_Index_Js.getInstance();
        vtigerInstance.showQuickPreviewForIdFilter(filtertab, recordId, 'Invoice', appName);
    },

    //unicnfsecrm_mod_15 ( quickView )
    registerEventToShowQuickPreview: function () {
        var self = this;
        var listViewPageDiv = $('.dashBoardContainer ');
        this.getContainer().on('click', '.quickView', function (e) {
            var element = listViewPageDiv.find(e.currentTarget);
            var recordId = element.attr('id');
            var app = element.data('app');
            var row = element.closest('.listViewEntries');

            var el = e.target;
            var filtertab = $(el).attr('title');
            self.showQuickPreviewForIdFilter(filtertab, recordId, app);
        });
    },
