    ...
    <div class="lineitemTableContainer" >
        <div class="fieldBlockContainer">            
            <table class="table table-bordered lineItemTabDate" id="lineItemTabDate">
                <tr>
                    <th colspan="9">
                        <span class="eventLineItemHeader">Veuillez indiquer les horaires de la formation.</span>            
                    </th>        
                </tr>
                <tr id="rowdate0" class="hide lineItemCloneCopyDates" data-row-num="0">
                        {include file="partials/LineItemsContentDates.tpl"|@vtemplate_path:'Events' row_no=0 data=[]}
                </tr>     
            {if count($RELATED_SESSION_DATES) eq 0}
                <tr id="rowdate1" class="lineItemRowDate" data-row-num="1">
                        {include file="partials/LineItemsContentDates.tpl"|@vtemplate_path:'Events' row_no=1 data=[]}
                </tr>
            {else}
                {assign var="date_id_value" value=""}        
                {assign var="rowlist_no" value=0} 
            {foreach key=row_no item=data from=$RELATED_SESSION_DATES name=stobjfor}       
                ...
                <tr id="rowdate{$row_no}" class="lineItemRowDate" data-row-num="{$row_no}">
                    {include file="partials/LineItemsContentDates.tpl"|@vtemplate_path:'Events' row_no=$row_no data=$data}
                </tr>       
            {/foreach}      
            {/if}
             </table>       
