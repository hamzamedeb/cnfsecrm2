{strip}
{assign var="FINAL" value=$RELATED_SESSION_APPRENANTS.1.final_details}
...
<div name='editContent' id="lineItemTabglobalApprenant">
	...
	<table class="table table-bordered lineItemTableApprenant" id="lineItemTabApprenant">
            ...
            <tr>
                    ...
                    <td><strong>{vtranslate('Statut facture',$MODULE)}</strong></td> 
                    <td><strong>{vtranslate('',$MODULE)}</strong></td>  
                    <td><strong>{vtranslate('',$MODULE)}</strong></td>
            </tr>
            <tr id="rowApprenant0" class="hide lineItemCloneCopyApprenant" data-row-num="0">
                    {include file="partials/LineItemsContentApprenants.tpl"|@vtemplate_path:'Events' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
            </tr>
            {foreach key=row_no item=data from=$RELATED_SESSION_APPRENANTS}
                    <tr id="rowApprenant{$row_no}" data-row-num="{$row_no}" class="lineItemRowApprenant">
                            {include file="partials/LineItemsContentApprenants.tpl"|@vtemplate_path:'Events' row_no=$row_no data=$data}
                    </tr>
            {/foreach}
            {if count($RELATED_SESSION_APPRENANTS) eq 0}
                    <tr id="rowApprenant1" class="lineItemRowApprenant" data-row-num="1">
                            {include file="partials/LineItemsContentApprenants.tpl"|@vtemplate_path:'Events' row_no=1 data=[] IGNORE_UI_REGISTRATION=false}
                    </tr>
            {/if}
        </table>
